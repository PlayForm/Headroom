...
pub fn compare_fixture(comparator: &dyn TransformComparator, fixture: &Fixture) -> Result<ComparisonOutcome> {
	let actual = match comparator.run(&fixture.input, &fixture.config) {
		Ok(v) => v,
		Err(e) => return Ok(ComparisonOutcome::Skipped { reason: format!("comparator error: {e}") }),
	};
	let actual_normalized: serde_json::Value = serde_json::from_str(&serde_json::to_string(&actual)?)
...
macro_rules! stub_comparator {
	($ty:ident, $name:literal) => {
		...
		impl TransformComparator for $ty {
			...
			fn run(&self, _input: &serde_json::Value, _config: &serde_json::Value) -> Result<serde_json::Value> {
...
impl TransformComparator for DiffCompressorComparator {
	...
	fn run(&self, input: &serde_json::Value, config: &serde_json::Value) -> Result<serde_json::Value> {
		...
		let content = input.as_str().context("diff_compressor fixture input must be a JSON string")?;
		...
		let cfg = DiffCompressorConfig {
			...
			max_files: config
				...
				.map(|v| v as usize)
				.unwrap_or(20),
			always_keep_additions: config.get("always_keep_additions").and_then(|v| v.as_bool()).unwrap_or(true),
			always_keep_deletions: config.get("always_keep_deletions").and_then(|v| v.as_bool()).unwrap_or(true),
			enable_ccr: config.get("enable_ccr").and_then(|v| v.as_bool()).unwrap_or(true),
...
impl TransformComparator for TokenizerComparator {
	...
	fn run(&self, input: &serde_json::Value, _config: &serde_json::Value) -> Result<serde_json::Value> {
		use headroom_core::tokenizer::{TiktokenCounter, Tokenizer};
		let text = input.as_str().context("tokenizer fixture input must be a JSON string")?;
		let counter = TiktokenCounter::for_model("gpt-4o-mini").context("init TiktokenCounter for gpt-4o-mini")?;
...
impl TransformComparator for SmartCrusherComparator {
	...
	fn run(&self, input: &serde_json::Value, config: &serde_json::Value) -> Result<serde_json::Value> {
		...
		let cfg = SmartCrusherConfig {
			enabled: config.get("enabled").and_then(|v| v.as_bool()).unwrap_or(defaults.enabled),
...
impl TransformComparator for ContentDetectorComparator {
	...
	fn run(&self, input: &serde_json::Value, _config: &serde_json::Value) -> Result<serde_json::Value> {
		...
		let content = input.as_str().context("content_detector fixture input must be a JSON string")?;
...
mod tests {
	...
	fn write_fixture(dir: &Path, transform: &str, name: &str, output: serde_json::Value) {
		...
		f.write_all(&serde_json::to_vec_pretty(&fixture).unwrap()).unwrap();
	}

	#[test]
	fn harness_reports_diff_for_divergent_comparator() {
		let tmp = tempdir();
		write_fixture(tmp.path(), "fake_divergent", "case1", serde_json::json!("python-output"));
	...
	fn harness_reports_match_for_agreeing_comparator() {
		let tmp = tempdir();
		write_fixture(tmp.path(), "fake_agreeing", "case1", serde_json::json!("python-output"));
	...
	fn stub_comparators_skip_rather_than_panic() {
		let tmp = tempdir();
		write_fixture(tmp.path(), "log_compressor", "case1", serde_json::json!({"compressed": "x"}));
	...
	fn tempdir() -> TempDir {
		...
		let p = std::env::temp_dir().join(format!("headroom-parity-{nanos}-{:?}", std::thread::current().id()));
