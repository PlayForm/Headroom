...
impl LegacyMutexStore {
	fn new(capacity: usize, ttl: Duration) -> Self {
		Self {
			inner: Mutex::new(LegacyInner { map: HashMap::new(), order: VecDeque::new() }),
			ttl,
			capacity,
		}
	}
...
impl CcrStore for LegacyMutexStore {
	fn put(&self, hash: &str, payload: &str) -> bool {
		...
		if g.map.contains_key(hash) {
			g.map.insert(
				hash.to_string(),
				LegacyEntry { payload: payload.to_string(), inserted: Instant::now() },
			);
			return true;
		}
	...
	fn del(&self, hash: &str) -> bool {
		self.inner.lock().unwrap().map.remove(hash).is_some()
	}
}
...
fn bench_mixed_multi_threaded(c: &mut Criterion) {
	...
	for &threads in &[1usize, 2, 4, 8] {
		...
		group.bench_function(&label, |b| {
			b.iter_custom(|n| {
				let store: Arc<dyn CcrStore> = Arc::new(LegacyMutexStore::new(1000, Duration::from_secs(300)));
				run_mt_workload(store, threads, n)
			});
		});
	}
