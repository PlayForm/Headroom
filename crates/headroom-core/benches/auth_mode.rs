...
fn bench_classify(c: &mut Criterion) {
	...
	let payg = build_headers(&[("authorization", "Bearer sk-ant-api03-abcdefghijklmnopqrstuvwxyz0123456789")]);
	group.bench_function("payg_anthropic_api_key", |b| b.iter(|| classify(black_box(&payg))));
	...
	let subscription = build_headers(&[
		...
		("authorization", "Bearer sk-ant-oat-01-abcdefghijklmnopqrstuvwxyz"),
		("content-type", "application/json"),
		("accept", "application/json"),
		("host", "api.anthropic.com"),
	]);
	group.bench_function("subscription_claude_code", |b| b.iter(|| classify(black_box(&subscription))));
