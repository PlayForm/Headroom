...
fn bench_count_text(c: &mut Criterion) {
	...
	group.bench_function("small", |b| {
		b.iter_batched(|| small, |s| black_box(counter.count_text(s)), BatchSize::SmallInput)
	});

	group.throughput(Throughput::Bytes(medium.len() as u64));
	group.bench_function("medium", |b| {
		b.iter_batched(|| medium.as_str(), |s| black_box(counter.count_text(s)), BatchSize::SmallInput)
	});

	group.throughput(Throughput::Bytes(large.len() as u64));
	group.bench_function("large", |b| {
		b.iter_batched(|| large.as_str(), |s| black_box(counter.count_text(s)), BatchSize::SmallInput)
	});
