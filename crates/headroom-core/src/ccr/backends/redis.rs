...
impl RedisCcrStore {
	...
	pub fn open_with_prefix(url: &str, key_prefix: String, default_ttl_seconds: u64) -> redis::RedisResult<Self> {
		...
		Ok(Self { client, key_prefix, default_ttl_seconds })
	}
...
impl CcrStore for RedisCcrStore {
	fn put(&self, hash: &str, payload: &str) -> bool {
		...
		let res: redis::RedisResult<()> = conn.set_ex(&key, payload.as_bytes(), self.default_ttl_seconds);
		match res {
			Ok(_) => true,
			Err(err) => {
				tracing::warn!(
