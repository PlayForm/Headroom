...
impl CcrBackendConfig {
	...
	pub fn sqlite_default(path: PathBuf) -> Self {
		Self::Sqlite { path, ttl_seconds: crate::ccr::DEFAULT_TTL.as_secs() }
	}
...
pub enum CcrBackendInitError {
	...
	UnsupportedBackend { backend: &'static str, feature: &'static str },
}
...
pub fn from_config(config: &CcrBackendConfig) -> Result<Box<dyn CcrStore>, CcrBackendInitError> {
	match config {
		CcrBackendConfig::InMemory { capacity, ttl_seconds } => {
			let store =
				InMemoryCcrStore::with_capacity_and_ttl(*capacity, std::time::Duration::from_secs(*ttl_seconds));
			tracing::info!(
				target = "ccr.backend",
				backend = "in_memory",
				capacity = *capacity,
		...
		#[cfg(feature = "redis")]
		CcrBackendConfig::Redis { url, ttl_seconds, key_prefix } => {
		...
		CcrBackendConfig::Redis { .. } => Err(CcrBackendInitError::UnsupportedBackend { backend: "redis", feature: "redis" }),
	}
}
