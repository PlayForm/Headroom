...
impl Default for EmbeddingScorer {
	...
	fn default() -> Self {
		EmbeddingScorer { model_name: "BAAI/bge-small-en-v1.5".to_string(), model: None }
	}
}
...
impl EmbeddingScorer {
	...
	pub fn try_new_with_model(model_kind: EmbeddingModel) -> Result<Self, String> {
		...
		Ok(EmbeddingScorer { model_name: name, model: Some(Mutex::new(model)) })
	}
}
...
impl RelevanceScorer for EmbeddingScorer {
	fn score(&self, item: &str, context: &str) -> RelevanceScore {
		...
		RelevanceScore::new(sim, format!("Embedding: semantic similarity {:.2}", sim), Vec::new())
	}
