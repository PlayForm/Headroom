...
pub fn detect_id_field_statistically(stats: &FieldStats, values: &[Value]) -> (bool, f64) {
	...
	if stats.field_type == "string" {
		...
		if !sample_values.is_empty() {
			...
			let avg_entropy =
				sample_values.iter().map(|s| calculate_string_entropy(s)).sum::<f64>() / sample_values.len() as f64;
...
pub fn detect_score_field_statistically(stats: &FieldStats, items: &[Value]) -> (bool, f64) {
	...
	let float_count = first_20.iter().filter(|&&v| v.is_finite() && v != v.trunc()).count();
...
mod tests {
	...
	fn id_field_uuid_strings_high_confidence() {
		...
		let values: Vec<Value> = (0..20).map(|i| json!(format!("550e8400-e29b-41d4-a716-{:012x}", i))).collect();
	...
	fn id_field_high_entropy_strings() {
		...
		let values: Vec<Value> = (0..20).map(|i| json!(format!("a3f7b2c{:06x}d8e1f4a7", i))).collect();
		let (is_id, conf) = detect_id_field_statistically(&s, &values);
		assert!(is_id);
	...
	fn score_field_unit_range_with_descending_sort() {
		...
		let items: Vec<Value> = (0..10).rev().map(|i| json!({"score": (i as f64) / 10.0})).collect();
	...
	fn score_field_signed_similarity_range() {
		...
		let items: Vec<Value> = (0..10).rev().map(|i| json!({"similarity": (i as f64) / 10.0 - 0.5})).collect();
		let (is_score, _) = detect_score_field_statistically(&s, &items);
		assert!(is_score);
	}
	...
	fn score_field_non_numeric_rejected() {
		...
		let items: Vec<Value> = vec![json!({"name": "alice"}), json!({"name": "bob"}), json!({"name": "alice"})];
		let (is_score, _) = detect_score_field_statistically(&s, &items);
		assert!(!is_score);
	}
	...
	fn score_field_confidence_capped_at_95() {
		...
		let s = stats_with_range("score", 0.0, 1.0);
		let items: Vec<Value> = (0..50).rev().map(|i| json!({"score": (i as f64) / 50.0})).collect();
