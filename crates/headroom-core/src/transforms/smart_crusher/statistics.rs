...
fn python_int_parse(s: &str) -> Option<i64> {
	...
	let cleaned: String = if trimmed.contains('_') {
		...
		let starts_or_ends = bytes[0] == b'_' || *bytes.last().unwrap() == b'_' || trimmed.contains("__");
		if starts_or_ends {
			return None;
		}
		trimmed.replace('_', "")
...
mod tests {
	...
	fn sequential_random_numbers_not_detected() {
		let v: Vec<Value> = vec![json!(100), json!(2), json!(85), json!(7), json!(43), json!(17)];
		assert!(!detect_sequential_pattern(&v, true));
	}
	...
	fn sequential_bools_excluded() {
		...
		let v = vec![json!(true), json!(false), json!(true), json!(false), json!(true), json!(false)];
		assert!(!detect_sequential_pattern(&v, true));
	}
	...
	fn bug2_all_unparseable_strings_returns_false() {
		...
		let v: Vec<Value> = vec![json!("abc"), json!("def"), json!("ghi"), json!("jkl"), json!("mno")];
		assert!(!detect_sequential_pattern(&v, true));
	}
	...
	fn sequential_with_whitespace_padded_strings_via_python_int_parse() {
		...
		let v: Vec<Value> = vec![json!(1), json!("  2  "), json!(3), json!(" 4 "), json!(5), json!(6)];
		assert!(detect_sequential_pattern(&v, true));
	}
}
