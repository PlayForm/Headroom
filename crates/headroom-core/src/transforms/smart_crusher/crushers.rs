...
pub fn compute_k_split(items: &[&str], config: &SmartCrusherConfig, bias: f64) -> (usize, usize, usize, usize) {
...
/// `compute_optimal_k`.
pub fn crush_string_array(items: &[&str], config: &SmartCrusherConfig, bias: f64) -> (Vec<String>, String) {
	let n = items.len();
	if n <= 8 {
		return (
...
pub fn crush_number_array(items: &[Value], config: &SmartCrusherConfig, bias: f64) -> (Vec<Value>, String) {
	let n = items.len();
	if n <= 8 {
	...
	let finite: Vec<f64> = items.iter().filter_map(|v| v.as_f64().filter(|f| f.is_finite())).collect();
	if finite.is_empty() {
	...
	let std_val = if finite.len() > 1 { sample_stdev(&finite).unwrap_or(0.0) } else { 0.0 };
	...
	if config.preserve_change_points && n > 10 {
		...
		for i in window..n.saturating_sub(window) {
			...
			if !left.is_empty() && !right.is_empty() {
				...
				if std_val > 0.0 && (right_mean - left_mean).abs() > config.variance_threshold * std_val {
...
pub fn crush_object(obj: &Map<String, Value>, config: &SmartCrusherConfig, bias: f64) -> (Map<String, Value>, String) {
	let n = obj.len();
	if n <= 8 {
	...
	if remaining > 0 {
		...
		while i < n {
			...
			let error_kept_count = keep_keys
				.iter()
				.filter(|k| {
					let s = serde_json::to_string(&obj[k.as_str()]).unwrap_or_default().to_lowercase();
...
mod tests {
	...
	fn string_array_keeps_error_strings() {
		let items: Vec<&str> = (0..30).map(|i| if i == 15 { "FATAL: out of memory" } else { "ok" }).collect();
		let (out, strat) = crush_string_array(&items, &cfg(), 1.0);
	...
	fn string_array_dedup_count_appears_in_strategy() {
		...
		// seen → dedup_count > 0.
		assert!(strat.contains("dedup="), "strategy {} should mention dedup", strat);
	}
	...
	fn object_keeps_error_keywords() {
		let mut obj = Map::new();
		obj.insert("msg1".to_string(), json!(format!("FATAL: {}", "x".repeat(200))));
		for i in 0..30 {
			obj.insert(
				format!("k{:02}", i),
		...
		let (out, _) = crush_object(&obj, &cfg(), 1.0);
		assert!(out.contains_key("msg1"), "key with error-keyword value must survive");
	}
