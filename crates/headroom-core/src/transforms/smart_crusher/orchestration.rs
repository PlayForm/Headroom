...
pub fn deduplicate_indices_by_content(keep_indices: &BTreeSet<usize>, items: &[Value]) -> BTreeSet<usize> {
...
pub fn prioritize_indices(
	...
	let error_indices: BTreeSet<usize> = detect_error_items_for_preservation(items, None).into_iter().collect();
...
mod tests {
	...
	fn dedup_lowest_index_wins_for_duplicates() {
		let items = vec![json!({"name": "alice"}), json!({"name": "alice"}), json!({"name": "bob"})];
	...
	fn prioritize_keeps_error_items_when_over_budget() {
		...
		let result = prioritize_indices(&cfg(), &kept, &items, items.len(), None, 10);
		assert!(result.contains(&30), "error item must survive prioritization");
	}
