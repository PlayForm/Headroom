...
pub use walker::{compact_document, emit_opaque_ccr_marker, try_parse_json_container, DocumentCompactor};
...
impl CompactionStage {
	...
	pub fn default_csv_schema() -> Self {
		Self { config: CompactConfig::default(), formatter: Box::new(CsvSchemaFormatter::new()) }
	}
	...
	pub fn csv_schema(config: CompactConfig) -> Self {
		Self { config, formatter: Box::new(CsvSchemaFormatter::new()) }
	}
	...
	pub fn default_json() -> Self {
		Self { config: CompactConfig::default(), formatter: Box::new(JsonFormatter::new()) }
	}
	...
	pub const SUPPORTED_FORMAT_NAMES: &'static [&'static str] = &["csv-schema", "json", "markdown-kv"];
