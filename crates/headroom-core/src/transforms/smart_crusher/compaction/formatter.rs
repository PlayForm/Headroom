...
fn compaction_to_json(c: &Compaction) -> Value {
	match c {
		Compaction::Table { schema, rows, original_count } => json!({
			"_compaction": "table",
		...
		Compaction::Buckets { discriminator, buckets, original_count } => json!({
			"_compaction": "buckets",
		...
		Compaction::OpaqueRef { ccr_hash, byte_size, kind } => json!({
			"_compaction": "ccr",
			"_hash": ccr_hash,
			"_size": byte_size,
...
fn cell_to_json(c: &CellValue) -> Value {
	match c {
		...
		CellValue::OpaqueRef { ccr_hash, byte_size, kind } => json!({
			"_ccr": ccr_hash,
			"_size": byte_size,
			"_kind": opaque_kind_str(kind),
		}),
...
fn write_compaction(out: &mut String, c: &Compaction, fmt: &CsvSchemaFormatter) {
	match c {
		...
		Compaction::OpaqueRef { ccr_hash, byte_size, kind } => {
...
fn write_table(out: &mut String, schema: &Schema, rows: &[Row], original_count: usize, fmt: &CsvSchemaFormatter) {
...
fn format_cell(c: &CellValue) -> String {
	match c {
		...
		CellValue::OpaqueRef { ccr_hash, byte_size, kind } => format_ccr_marker(ccr_hash, *byte_size, kind),
	}
}
...
fn format_ccr_marker(hash: &str, byte_size: usize, kind: &OpaqueKind) -> String {
	...
	format!("<<ccr:{},{},{}>>", hash, kind_str, humanize_bytes(byte_size))
}
...
fn write_kv_table(out: &mut String, schema: &Schema, rows: &[Row], original_count: usize, fmt: &MarkdownKvFormatter) {
...
mod tests {
	...
	fn json_formatter_renders_table() {
		let items = vec![json!({"id": 1, "name": "alice"}), json!({"id": 2, "name": "bob"})];
	...
	fn csv_formatter_quotes_strings_with_commas() {
		let items = vec![json!({"id": 1, "name": "alice, the great"}), json!({"id": 2, "name": "bob"})];
		let c = compact(&items, &cfg());
		let out = CsvSchemaFormatter::new().format(&c);
	...
	fn csv_formatter_escapes_internal_quotes() {
		let items = vec![json!({"id": 1, "msg": "she said \"hi\""}), json!({"id": 2, "msg": "ok"})];
		let c = compact(&items, &cfg());
		let out = CsvSchemaFormatter::new().format(&c);
	...
	fn csv_formatter_drop_summary_opt_in() {
		...
		let c = Compaction::Table {
			schema: Schema {
				fields: vec![super::super::ir::FieldSpec { name: "x".into(), type_tag: "int".into(), nullable: false }],
			},
			rows,
