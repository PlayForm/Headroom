...
pub fn emit_opaque_ccr_marker(payload: &str, kind: &OpaqueKind, store: Option<&Arc<dyn CcrStore>>) -> String {
...
mod tests {
	...
	fn stringified_json_in_field_is_parsed_and_compacted() {
		...
		let out = dc().compact(doc);
		let payload = out.pointer("/payload").and_then(|v| v.as_str()).expect("payload compacted");
	...
	fn long_opaque_string_at_top_level_becomes_ccr_marker() {
		...
		match out {
			Value::String(s) => assert!(s.starts_with("<<ccr:") && s.contains(",base64,"), "got: {s}"),
			other => panic!("expected String, got {other:?}"),
		}
	}
	...
	fn mixed_doc_only_compactable_parts_change() {
		...
		assert!(out.pointer("/config").map(|v| v.is_object()).unwrap_or(false));
		// events compacted to a string.
		assert!(out.pointer("/events").and_then(|v| v.as_str()).unwrap().starts_with("[2]{"));
	}
