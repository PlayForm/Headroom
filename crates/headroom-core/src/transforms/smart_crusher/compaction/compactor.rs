...
pub fn compact_with_store(items: &[Value], cfg: &CompactConfig, store: Option<&Arc<dyn CcrStore>>) -> Compaction {
	compact_inner(items, cfg, store)
}

fn compact_inner(items: &[Value], cfg: &CompactConfig, store: Option<&Arc<dyn CcrStore>>) -> Compaction {
	...
	let core_ratio = if total_keys == 0 { 1.0 } else { core_count as f64 / total_keys as f64 };
...
fn build_homogeneous_table(
	...
	let mut rows: Vec<Row> = items.iter().map(|item| build_row(item, &ordered_keys, cfg, store)).collect();

	flatten_uniform_nested(&mut field_specs, &mut rows, cfg);

	Compaction::Table { schema: Schema { fields: field_specs }, rows, original_count: items.len() }
}

fn build_row(item: &Value, ordered_keys: &[String], cfg: &CompactConfig, store: Option<&Arc<dyn CcrStore>>) -> Row {
...
fn cell_from_value(v: &Value, cfg: &CompactConfig, store: Option<&Arc<dyn CcrStore>>) -> CellValue {
	match classify_cell(v, &cfg.classify) {
		...
		CellClass::Opaque(kind) => {
			...
			CellValue::OpaqueRef { ccr_hash, byte_size: s.len(), kind }
		},
	}
}
...
fn flatten_uniform_nested(specs: &mut Vec<FieldSpec>, rows: &mut [Row], cfg: &CompactConfig) {
	...
	while i < specs.len() {
		...
		let new_specs: Vec<FieldSpec> = inner_keys
			.iter()
			.map(|k| FieldSpec { name: format!("{parent_name}.{k}"), type_tag: "string".into(), nullable: false })
			.collect();
		...
		for row in rows.iter_mut() {
			...
			let inner_obj: Option<serde_json::Map<String, Value>> = match original {
				...
				_ => unreachable!("uniform_object_keys guarantees every cell is Scalar(Object) or Missing"),
			};
...
/// non-trivial buckets.
fn detect_discriminator(items: &[Value], key_freqs: &BTreeMap<String, usize>, cfg: &CompactConfig) -> Option<String> {
	let total = items.len();
...
fn bucket_by(
	...
	let buckets: Vec<Bucket> = groups
		...
		.map(|(key, group_items)| {
			...
			match inner {
				Compaction::Table { schema, rows, .. } => Bucket { key: Value::String(key), schema, rows },
				_ => {
					...
					Bucket {
						...
						schema: Schema {
							fields: vec![FieldSpec { name: "value".into(), type_tag: "json".into(), nullable: false }],
						},
						rows: group_items.into_iter().map(|v| Row::new(vec![CellValue::Scalar(v)])).collect(),
					}
				},
			}
		})
		.collect();
	Compaction::Buckets { discriminator: discriminator.to_string(), buckets, original_count: items.len() }
}
...
mod tests {
	...
	fn pure_tabular_produces_table() {
		...
		match compact(&items, &cfg()) {
			Compaction::Table { schema, rows, original_count } => {
	...
	fn stringified_json_array_recurses() {
		...
		match compact(&items, &cfg()) {
			Compaction::Table { rows, .. } => {
				...
				let nested_count = [cell0, cell1].iter().filter(|c| matches!(***c, CellValue::Nested(_))).count();
				let _ = payload_idx;
	...
	fn opaque_cell_becomes_ccr_ref() {
		...
		let items = vec![json!({"id": 1, "blob": big.clone()}), json!({"id": 2, "blob": big.clone()})];
		match compact(&items, &cfg()) {
			Compaction::Table { rows, schema, .. } => {
				let blob_idx = schema.fields.iter().position(|f| f.name == "blob").expect("blob col");
				match &rows[0].0[blob_idx] {
					CellValue::OpaqueRef { ccr_hash, byte_size, kind } => {
	...
	fn heterogeneous_array_buckets_by_discriminator() {
		...
		match compact(&items, &cfg()) {
			Compaction::Buckets { discriminator, buckets, original_count } => {
	...
	fn nullable_field_marked() {
		let items = vec![json!({"id": 1, "tag": "a"}), json!({"id": 2}), json!({"id": 3, "tag": null})];
		match compact(&items, &cfg()) {
			Compaction::Table { schema, .. } => {
