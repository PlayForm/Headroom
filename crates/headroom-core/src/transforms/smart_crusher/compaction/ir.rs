...
pub enum CellValue {
	...
	OpaqueRef { ccr_hash: String, byte_size: usize, kind: OpaqueKind },
...
mod tests {
	...
	fn schema_field_names_returns_in_order() {
		let s = Schema {
			fields: vec![
				FieldSpec { name: "id".into(), type_tag: "int".into(), nullable: false },
				FieldSpec { name: "name".into(), type_tag: "string".into(), nullable: false },
			],
		};
