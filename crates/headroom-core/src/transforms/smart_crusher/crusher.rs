...
use super::compaction::{
	classify_cell, emit_opaque_ccr_marker, try_parse_json_container, CellClass, ClassifyConfig, CompactConfig,
...
impl SmartCrusher {
	...
	pub fn crush_with_prose_hook(&self, content: &str, query: &str, bias: f64, hook: &ProseHook<'_>) -> CrushResult {
		let start = std::time::Instant::now();
		let (compressed, was_modified, info) = self.smart_crush_content_with_hook(content, query, bias, Some(hook));
		let strategy = if info.is_empty() { "passthrough".to_string() } else { info };
	...
	pub fn with_scorer(config: SmartCrusherConfig, scorer: Box<dyn RelevanceScorer + Send + Sync>) -> Self {
		SmartCrusherBuilder::new(config)
			.with_scorer(scorer)
	...
	fn smart_crush_content_with_hook(
		...
		let (crushed, info) = self.process_value_with_hook(&parsed, 0, query_context, bias, prose_hook);
	...
	fn process_value_with_hook(
		...
		match value {
				...
				if n >= self.config.min_items_to_analyze {
					...
					match arr_type {
						ArrayType::DictArray => {
							...
							let result = self.crush_array_with_source(&rows, arr, query_context, bias);
			...
			Value::Object(map) => {
				...
				let mut processed = serde_json::Map::new();
				for (k, v) in map {
					let (p_val, p_info) = self.process_value_with_hook(v, depth + 1, query_context, bias, prose_hook);
					processed.insert(k.clone(), p_val);
					if !p_info.is_empty() {
						info_parts.push(p_info);
					}
			...
			Value::String(s) => self.process_string_with_hook(s, depth, query_context, bias, prose_hook),
	...
	fn process_string_with_hook(
		...
		if let CellClass::Opaque(kind) = classify_cell(&Value::String(s.to_string()), &cfg) {
			match kind {
				super::compaction::OpaqueKind::Base64Blob | super::compaction::OpaqueKind::HtmlChunk => {
				...
				super::compaction::OpaqueKind::LongString | super::compaction::OpaqueKind::Other(_) => {},
			}
		}
	...
	pub fn crush_mixed_array(&self, items: &[Value], query_context: &str, bias: f64) -> (Vec<Value>, String) {
		let n = items.len();
		if n <= 8 {
		...
		for (type_key, indices, values) in groups.into_iter() {
			...
			match type_key {
				"dict" => {
					let CrushArrayResult { items: crushed, .. } = self.crush_array(&values, query_context, bias);
				...
				"str" => {
					...
					let crushed_set: std::collections::HashSet<&str> = crushed.iter().map(|s| s.as_str()).collect();
					for (i, idx) in indices.iter().enumerate() {
				...
				"number" => {
					...
					let last_idx: Vec<usize> = indices.iter().rev().take(kl).copied().collect::<Vec<_>>();
					keep_indices.extend(&first_idx);
					keep_indices.extend(&last_idx);

					// Outliers via finite-only stats.
					let finite: Vec<f64> = values.iter().filter_map(|v| v.as_f64().filter(|f| f.is_finite())).collect();
					if finite.len() > 1 {
		...
		let strategy = format!("mixed:adaptive({}->{},{})", n, result.len(), strategy_parts.join(","));
		(result, strategy)
	}
}
...
mod tests {
	...
	fn execute_plan_returns_items_in_sorted_index_order() {
		...
		let plan = CompressionPlan { keep_indices: vec![5, 2, 8, 0], ..CompressionPlan::default() };
		let result = c.execute_plan(&plan, &items);
	...
	fn execute_plan_skips_out_of_bounds() {
		...
		let plan = CompressionPlan { keep_indices: vec![0, 5, 2], ..CompressionPlan::default() };
		let result = c.execute_plan(&plan, &items);
	...
	fn execute_plan_factors_constants_when_enabled() {
		let cfg = SmartCrusherConfig { factor_out_constants: true, ..Default::default() };
		...
		let plan = CompressionPlan { keep_indices: vec![0, 1, 2], constant_fields, ..CompressionPlan::default() };
		let result = c.execute_plan(&plan, &items);
	...
	fn execute_plan_keeps_drifted_values_when_factoring() {
		...
		let cfg = SmartCrusherConfig { factor_out_constants: true, ..Default::default() };
		let c = SmartCrusher::new(cfg);
		let items = vec![json!({"id": 0, "status": "ok"}), json!({"id": 1, "status": "FAILED"})];
		let mut constant_fields = std::collections::BTreeMap::new();
		constant_fields.insert("status".to_string(), json!("ok"));
		let plan = CompressionPlan { keep_indices: vec![0, 1], constant_fields, ..CompressionPlan::default() };
		let result = c.execute_plan(&plan, &items);
	...
	fn crush_array_skip_path_returns_original_items() {
		...
		let items: Vec<Value> = (0..30).map(|i| json!({"id": i, "name": format!("user_{}", i)})).collect();
		let result = c.crush_array(&items, "", 1.0);
	...
	fn crusher_with_custom_scorer() {
		...
		let c = SmartCrusher::with_scorer(SmartCrusherConfig::default(), Box::new(BM25Scorer::default()));
	...
	fn lossy_falls_through_when_savings_below_threshold() {
		...
		let cfg = SmartCrusherConfig { lossless_min_savings_ratio: 0.99, ..Default::default() };
		let c = SmartCrusher::new(cfg);
		...
		assert!(result.items.len() < 50, "expected lossy drop, got {} items", result.items.len());
	...
	fn enable_ccr_marker_false_suppresses_marker_and_store() {
		...
		let c = SmartCrusherBuilder::new(cfg).with_ccr_store(Arc::clone(&store)).build();
		let items: Vec<Value> = (0..50).map(|_| json!({"status": "ok"})).collect();
	...
	fn enable_ccr_marker_true_is_default_behavior() {
		...
		assert!(store_len_after > store_len_before, "default should write to ccr_store");
	}
	...
	fn enable_ccr_marker_false_suppresses_opaque_markers() {
		...
		let on =
			SmartCrusher::new(SmartCrusherConfig { lossless_min_savings_ratio: 0.0, ..SmartCrusherConfig::default() });
		let rendered_on = on
			.crush_array(&rows, "", 1.0)
			.compacted
			.expect("lossless table should ship at ratio 0.0");
	...
	fn lossless_only_leaves_array_uncompacted_instead_of_dropping() {
		...
		assert!(result.compacted.is_none(), "nothing shipped, nothing dropped");
	}
	...
	fn lossless_only_never_writes_to_ccr_store() {
		...
		assert_eq!(result.items, items, "lossless_only must keep every row (no drop)");
		assert!(result.ccr_hash.is_none(), "no hash under lossless_only");
		assert_eq!(
			store.len(),
			store_len_before,
