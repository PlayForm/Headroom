...
impl<'a> SmartCrusherPlanner<'a> {
	pub fn new(
		...
		SmartCrusherPlanner { config, anchor_selector, scorer, analyzer, constraints }
	}
	...
	fn apply_constraints(&self, items: &[Value], item_strings: Option<&[String]>, keep: &mut BTreeSet<usize>) {
	...
	pub fn create_plan(
		...
		match analysis.recommended_strategy {
			CompressionStrategy::TimeSeries => {
				self.plan_time_series(analysis, items, plan, query_context, preserve_fields, max_items, item_strings)
			},
			CompressionStrategy::ClusterSample => {
				self.plan_cluster_sample(analysis, items, plan, query_context, preserve_fields, max_items, item_strings)
			},
			CompressionStrategy::TopN => {
				self.plan_top_n(analysis, items, plan, query_context, preserve_fields, max_items, item_strings)
			},
			// SmartSample, None, Skip-already-handled, all fall here.
			_ => self.plan_smart_sample(analysis, items, plan, query_context, preserve_fields, max_items, item_strings),
		}
	}
	...
	pub fn plan_smart_sample(
		...
		let final_keep = prioritize_indices(self.config, &keep, items, n, Some(analysis), max_items);
	...
	pub fn plan_top_n(
		...
		if !query_context.is_empty() {
			...
			let strs: Vec<&str> = match item_strings {
				...
				None => {
					owned_strings = items.iter().map(|i| serde_json::to_string(i).unwrap_or_default()).collect();
	...
	pub fn plan_cluster_sample(
		...
		for (name, stats) in &analysis.field_stats {
			if stats.field_type == "string" && stats.unique_ratio > max_uniqueness && stats.unique_ratio > 0.3 {
...
pub fn item_has_preserve_field_match(item: &Value, preserve_field_hashes: &[String], query_context: &str) -> bool {
	if query_context.is_empty() {
		return false;
	}
...
fn for_each_anomaly(
	...
	for (i, item) in items.iter().enumerate() {
		if let Some(num) = item.as_object().and_then(|o| o.get(field_name)).and_then(|v| v.as_f64()) {
...
mod tests {
	...
	fn anchor_pattern_mapping_matches_python() {
		assert_eq!(map_to_anchor_pattern(CompressionStrategy::TimeSeries), DataPattern::TimeSeries);
		assert_eq!(map_to_anchor_pattern(CompressionStrategy::TopN), DataPattern::SearchResults);
		assert_eq!(map_to_anchor_pattern(CompressionStrategy::ClusterSample), DataPattern::Logs);
		assert_eq!(map_to_anchor_pattern(CompressionStrategy::SmartSample), DataPattern::Generic);
		assert_eq!(map_to_anchor_pattern(CompressionStrategy::None), DataPattern::Generic);
	}
	...
	fn preserve_field_match_query_substring_in_value() {
		...
		let fields = vec![h];
		assert!(item_has_preserve_field_match(&item, &fields, "find user alice please"));
	}
	...
	fn smart_sample_keeps_error_items() {
		let (cfg, asel, scorer, analyzer, cs) = make_planner_deps();
		let p = fixture(&cfg, &asel, &scorer, &analyzer, &cs);
		let mut items: Vec<Value> = (0..30).map(|i| json!({"id": i, "msg": format!("ok {}", i)})).collect();
		items.push(json!({"id": 30, "msg": "FATAL: out of memory"}));
		let analysis = analyzer.analyze_array(&items);
		let plan_in = CompressionPlan { strategy: CompressionStrategy::SmartSample, ..CompressionPlan::default() };
		let plan = p.plan_smart_sample(&analysis, &items, plan_in, "", None, 10, None);
		assert!(plan.keep_indices.contains(&30), "error item must survive plan_smart_sample");
	}
	...
	fn top_n_falls_back_when_no_score_field() {
		...
		let analysis = analyzer.analyze_array(&items);
		let plan_in = CompressionPlan { strategy: CompressionStrategy::TopN, ..CompressionPlan::default() };
	...
	fn top_n_keeps_highest_scored_items() {
		...
		let items: Vec<Value> = (0..20).map(|i| json!({"id": i, "score": (19 - i) as f64 * 0.05})).collect();
		let analysis = analyzer.analyze_array(&items);
		let plan_in = CompressionPlan { strategy: CompressionStrategy::TopN, ..CompressionPlan::default() };
		let plan = p.plan_top_n(&analysis, &items, plan_in, "", None, 10, None);
		// Top scores are at indices 0..7 (highest score = first item).
		assert!(plan.keep_indices.contains(&0), "highest-scored item (idx 0) should be kept");
	}
	...
	fn cluster_sample_assigns_cluster_field() {
		...
		let items: Vec<Value> = (0..30)
			...
			.collect();
		let analysis = analyzer.analyze_array(&items);
		let plan_in = CompressionPlan { strategy: CompressionStrategy::ClusterSample, ..CompressionPlan::default() };
	...
	fn time_series_keeps_window_around_change_points() {
		...
		let items: Vec<Value> = (0..60)
			...
			.collect();
		let analysis = analyzer.analyze_array(&items);
		let plan_in = CompressionPlan { strategy: CompressionStrategy::TimeSeries, ..CompressionPlan::default() };
