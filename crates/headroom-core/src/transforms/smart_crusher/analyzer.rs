...
impl SmartAnalyzer {
	...
	pub fn analyze_field(&self, key: &str, items: &[Value]) -> FieldStats {
		...
		let unique_ratio = if values.is_empty() { 0.0 } else { unique_count as f64 / values.len() as f64 };

		let is_constant = unique_count == 1;
		let constant_value = if is_constant { Some(non_null[0].clone()) } else { None };
		...
		match field_type.as_str() {
			"numeric" => {
				...
				let nums: Vec<f64> = non_null.iter().filter_map(|v| v.as_f64().filter(|f| f.is_finite())).collect();
				if !nums.is_empty() {
					...
					let variance = if nums.len() > 1 { sample_variance(&nums) } else { Some(0.0) };
	...
	pub fn detect_pattern(&self, field_stats: &BTreeMap<String, FieldStats>, items: &[Value]) -> String {
	...
	pub fn detect_temporal_field(&self, field_stats: &BTreeMap<String, FieldStats>, items: &[Value]) -> bool {
		for (name, stats) in field_stats {
			match stats.field_type.as_str() {
				"string" => {
					...
					let iso_count = sample.iter().filter(|s| is_iso_datetime(s) || is_iso_date(s)).count();
	...
	pub fn analyze_crushability(
		...
		let string_ratios: Vec<f64> = field_stats
			...
			.collect();
		let avg_string_uniqueness = if string_ratios.is_empty() { 0.0 } else { mean(&string_ratios).unwrap_or(0.0) };
		...
		if non_id_content_uniqueness < 0.1 && has_id_field {
			...
			return make(true, 0.85, "repetitive_content_with_ids", sp, signals_absent);
		}

		// Case 1: low uniqueness.
		if max_uniqueness < 0.3 {
			return make(true, 0.9, "low_uniqueness_safe_to_sample", signals_present, signals_absent);
		}

		// Case 2: high uniqueness + ID field + NO signal = DON'T CRUSH.
		if has_id_field && max_uniqueness > 0.8 && !has_any_signal {
			return make(false, 0.85, "unique_entities_no_signal", signals_present, signals_absent);
		}

		// Case 3: high uniqueness + has signal = crush.
		if max_uniqueness > 0.8 && has_any_signal {
			return make(true, 0.7, "unique_entities_with_signal", signals_present, signals_absent);
		}

		// Case 4: medium uniqueness + no signal = don't crush.
		if !has_any_signal {
			return make(false, 0.6, "medium_uniqueness_no_signal", signals_present, signals_absent);
		}

		// Case 5: medium uniqueness + has signal = crush with caution.
		make(true, 0.5, "medium_uniqueness_with_signal", signals_present, signals_absent)
	}
...
fn top_n_by_count(strs: &[&str], n: usize) -> Vec<(String, usize)> {
	...
	pairs.into_iter().take(n).map(|(k, c)| ((*k).to_string(), c)).collect()
}
...
mod tests {
	...
	fn temporal_iso_date() {
		let items: Vec<Value> = (1..=10).map(|i| json!({"d": format!("2025-01-{:02}", i)})).collect();
		let a = analyzer();
		let mut fs: BTreeMap<String, FieldStats> = BTreeMap::new();
	...
	fn temporal_iso_datetime() {
		let items: Vec<Value> = (1..=10).map(|i| json!({"t": format!("2025-01-{:02}T12:00:00Z", i)})).collect();
		let a = analyzer();
		let mut fs: BTreeMap<String, FieldStats> = BTreeMap::new();
	...
	fn temporal_unix_seconds_range() {
		...
		let items: Vec<Value> = (0..10).map(|i| json!({"ts": 1_700_000_000_i64 + i * 86400})).collect();
		let a = analyzer();
		let mut fs: BTreeMap<String, FieldStats> = BTreeMap::new();
	...
	fn crushability_unique_entities_no_signal_skips() {
		...
		let items: Vec<Value> = (0..20).map(|i| json!({"id": i, "name": format!("user_{}", i)})).collect();
		let a = analyzer();
		let mut fs: BTreeMap<String, FieldStats> = BTreeMap::new();
		for k in ["id", "name"] {
			fs.insert(k.to_string(), a.analyze_field(k, &items));
