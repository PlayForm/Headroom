...
impl ReformatTransform for JsonMinifier {
	...
	fn apply(&self, content: &str) -> Result<ReformatOutput, TransformError> {
		...
		let value: serde_json::Value =
			serde_json::from_str(trimmed).map_err(|e| TransformError::invalid_input(NAME, e.to_string()))?;

		let minified = serde_json::to_string(&value).map_err(|e| TransformError::internal(NAME, e.to_string()))?;
