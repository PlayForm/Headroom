...
impl LogTemplate {
	...
	fn flush_run(
		...
		let collapse =
			run.indices.len() >= cfg.min_run && constant_count >= cfg.min_constant_tokens && varying_count > 0;
...
mod tests {
	...
	fn lossless_round_trip_via_template_and_variants() {
		...
		let var_pos = template_tokens.iter().position(|t| *t == WILDCARD).expect("must have wildcard");
	...
	fn all_unique_lines_are_emitted_verbatim() {
		...
		for i in 0..25 {
			assert!(r.output.contains(&format!("event-{i}")), "missing event-{i} in output");
		}
	}
	...
	fn blank_lines_break_runs() {
		...
		if t1_count == 1 && t2_count == 0 {
			assert!(!r.output.contains("(10 occurrences)"), "must not bridge the blank line");
		}
	}
