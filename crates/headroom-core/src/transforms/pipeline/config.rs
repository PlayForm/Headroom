...
impl PipelineConfig {
	...
	pub fn from_default_str() -> Self {
		toml::from_str(DEFAULT_TOML).expect("embedded config/pipeline.toml must parse — checked by tests")
	}
...
mod tests {
	...
	fn bloat_log_weights_sum_to_at_most_one() {
		...
		assert!(total <= 1.0001, "log bloat weights must sum to ≤ 1.0, got {total}");
	}
	...
	fn from_toml_str_overrides_defaults() {
		...
		assert_eq!(cfg.offload.diff_noise.lockfile_suffixes, vec!["custom.lock"]);
	}
	...
	fn defaults_carry_reformat_and_offload_sections() {
		...
		assert!(cfg.offload.diff_noise.lockfile_suffixes.iter().any(|s| s == "Cargo.lock"));
	}
