...
use crate::transforms::pipeline::traits::{CompressionContext, OffloadTransform, ReformatTransform, TransformError};
...
impl CompressionPipeline {
	...
	pub fn run(
		...
		if original_len == 0 {
			return PipelineResult { output: String::new(), ..Default::default() };
		}

		let empty_reformats: Vec<Arc<dyn ReformatTransform>> = Vec::new();
		let empty_offloads: Vec<Arc<dyn OffloadTransform>> = Vec::new();
		let reformats = self.reformats_by_type.get(&content_type).unwrap_or(&empty_reformats);
		let offloads = self.offloads_by_type.get(&content_type).unwrap_or(&empty_offloads);
		...
		for (offload, score) in offloads.iter().zip(bloat_scores.iter()) {
			...
			let reformat_underwhelmed = reformat_ratio > self.config.pipeline.offload_fallback_ratio && *score > 0.0;
		...
		PipelineResult { output: current, bytes_saved: total_saved, steps_applied: steps, cache_keys }
	}

	/// Run reformat transforms in registration order against `content`.
	/// Stops once `current_len / original_len <= reformat_target_ratio`.
	fn run_reformats(&self, content: &str, reformats: &[Arc<dyn ReformatTransform>]) -> ReformatAccumulator {
		let original_len = content.len();
		...
		ReformatAccumulator { output: current, bytes_saved: total_saved, steps }
	}
	...
	fn estimate_bloats(&self, content: &str, offloads: &[Arc<dyn OffloadTransform>]) -> Vec<f32> {
		offloads.par_iter().map(|o| o.estimate_bloat(content)).collect()
	}
...
mod tests {
	...
	use crate::transforms::pipeline::offloads::{DiffNoise, DiffOffload, JsonOffload, LogOffload, SearchOffload};
	...
	fn test_offload(name: &'static str, score: f32) -> TestOffload {
		TestOffload { score, applies_to: vec![ContentType::PlainText], confidence_score: 0.5, name }
	}
	...
	fn offload_skipped_when_bloat_below_threshold_and_reformat_was_enough() {
		...
		let s = store();
		let r = p.run("x".repeat(100).as_str(), ContentType::PlainText, &ctx(), &s);
		assert!(r.steps_applied.is_empty(), "score 0.0 should never run: {:?}", r.steps_applied);
		assert_eq!(s.len(), 0);
	}
	...
	impl ReformatTransform for AlwaysHalf {
		...
		fn apply(&self, content: &str) -> Result<ReformatOutput, TransformError> {
			let half = &content[..content.len() / 2];
			Ok(ReformatOutput::from_lengths(content.len(), half.to_string()))
		}
	}
	...
	fn offload_above_threshold_runs_even_when_reformat_was_great() {
		...
		let s = store();
		let r = p.run("x".repeat(100).as_str(), ContentType::PlainText, &ctx(), &s);
		assert_eq!(r.steps_applied, vec!["always_half".to_string(), "forced".to_string()]);
		assert_eq!(r.cache_keys.len(), 1);
	}
	...
	fn end_to_end_diff_offload_compresses_context_heavy_diff() {
		...
		diff.push_str("diff --git a/x.txt b/x.txt\n--- a/x.txt\n+++ b/x.txt\n@@ -1,105 +1,105 @@\n");
		for _ in 0..100 {
	...
	fn end_to_end_json_minifier_then_json_offload_on_tabular_array() {
		...
		for i in 0..200 {
			...
			input.push_str(&format!("  {{\"id\": {i}, \"name\": \"event-{i}\", \"value\": {}}}", i * 100));
		}
		input.push_str("\n]");
