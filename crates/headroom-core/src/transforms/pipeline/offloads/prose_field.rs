...
impl ProseFieldOffload {
	pub fn new(config: ProseFieldConfig) -> Self {
		Self { crusher: TextCrusher::default(), config }
	}
	...
	fn eligible(&self, content: &str) -> bool {
		content.len() >= self.config.min_bytes && detect_content_type(content).content_type == ContentType::PlainText
	}
	...
	fn compress(&self, content: &str, query: &str) -> Option<(String, String)> {
		...
		let result = self.crusher.compress(content, query, Some(self.config.target_ratio));
...
impl OffloadTransform for ProseFieldOffload {
	...
	fn estimate_bloat(&self, content: &str) -> f32 {
		...
		let segments = content
			...
			.count();
		if segments < self.config.min_segments { 0.0 } else { 1.0 }
	}
	...
	fn apply(
		...
		let Some((output, key)) = self.compress(content, &ctx.query) else {
			return Err(TransformError::skipped(NAME, "prose compression not worth it"));
		};
