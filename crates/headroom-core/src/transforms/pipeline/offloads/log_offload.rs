...
impl LogOffload {
	...
	pub fn with_compressor(
		...
		Self { compressor, bloat, detector, bias: 0.0 }
	}
...
impl OffloadTransform for LogOffload {
	...
	fn estimate_bloat(&self, content: &str) -> f32 {
		...
		let score = repetition * self.bloat.uniqueness_weight + dilution * self.bloat.priority_dilution_weight;
		score.clamp(0.0, 1.0)
	}
	...
	fn apply(
		...
		let (result, stats) = self.compressor.compress_with_store(content, self.bias, Some(store));
...
mod tests {
	...
	fn estimate_bloat_unique_errors_score_low() {
		...
		let lines: Vec<String> = (0..100).map(|i| format!("ERROR: failure number {i} at module x")).collect();
		let log = lines.join("\n");
		let score = offload().estimate_bloat(&log);
	...
	fn estimate_bloat_priority_dilution_alone_scores_meaningfully() {
		...
		let lines: Vec<String> = (0..100).map(|i| format!("INFO: routine event #{i}")).collect();
		let log = lines.join("\n");
		let score = offload().estimate_bloat(&log);
