...
impl SearchOffload {
	pub fn new(bloat: SearchBloatConfig) -> Self {
		Self::with_compressor(SearchCompressor::new(SearchCompressorConfig::default()), bloat)
	}

	pub fn with_compressor(compressor: SearchCompressor, bloat: SearchBloatConfig) -> Self {
		Self { compressor, bloat, bias: 0.0 }
	}
...
impl OffloadTransform for SearchOffload {
	...
	fn apply(
		...
		let (result, stats) = self.compressor.compress_with_store(content, &ctx.query, self.bias, Some(store));
		...
		Ok(OffloadOutput::from_lengths(content.len(), result.compressed, key))
	}
...
fn extract_file_prefix(line: &str) -> Option<&str> {
	...
	let scan_start = if bytes.len() >= 2 && bytes[1] == b':' && (bytes[0] as char).is_ascii_alphabetic() {
		2
	} else {
		0
	};
...
mod tests {
	...
	fn extract_file_prefix_handles_grep() {
		assert_eq!(extract_file_prefix("src/utils.py:42:def foo():"), Some("src/utils.py"));
	}

	#[test]
	fn extract_file_prefix_handles_ripgrep_context() {
		assert_eq!(extract_file_prefix("src/main.py-43-some context"), Some("src/main.py"));
	}
	...
	fn estimate_bloat_distributed_matches_score_low() {
		...
		let s: String = (0..20).map(|i| format!("file{i}.py:1:line")).collect::<Vec<_>>().join("\n");
		let score = offload().estimate_bloat(&s);
		assert_eq!(score, 0.0);
	}
