...
impl OffloadTransform for DiffOffload {
	...
	fn apply(
		...
		let (result, _) = self.compressor.compress_with_store(content, &ctx.query, Some(store));

		let Some(key) = result.cache_key else {
			return Err(TransformError::skipped(NAME, "diff compressor did not emit a cache_key"));
		};
...
mod tests {
	...
	fn estimate_bloat_context_heavy_diff_scores_high() {
		...
		let score = offload().estimate_bloat(&diff);
		assert!(score > 0.7, "context-heavy diff should score high, got {score}");
	}
