...
impl OffloadTransform for DiffNoise {
	...
	fn estimate_bloat(&self, content: &str) -> f32 {
		...
		for seg in &segments {
			let body_bytes: usize = seg.body_lines.iter().map(|l| l.len() + 1 /* the '\n' */).sum();
	...
	fn apply(
		...
		for seg in &segments {
			...
			let drop_whitespace = self.config.drop_whitespace_only_hunks && seg.body_is_whitespace_only();
...
fn parse_segments(content: &str) -> Vec<Segment<'_>> {
	...
	for line in content.lines() {
		if line.starts_with("diff --git") {
			...
			current =
				Some(Segment { new_path: parse_new_path(line), header_lines: vec![line], body_lines: Vec::new() });
			in_body = false;
			continue;
		}
...
mod tests {
	...
	fn estimate_bloat_lockfile_dominates_scores_high() {
		...
		let big_lock_body: Vec<String> = (0..200).flat_map(|i| vec![format!("-old{i}"), format!("+new{i}")]).collect();
		let body_refs: Vec<&str> = big_lock_body.iter().map(|s| s.as_str()).collect();
		let diff = build_diff(&[("Cargo.lock", &body_refs), ("Cargo.toml", &["-foo = \"1\"", "+foo = \"2\""])]);
		let score = offload().estimate_bloat(&diff);
		assert!(score > 0.9, "lockfile-dominated diff should score high, got {score}");
	}
	...
	fn estimate_bloat_whitespace_only_hunk_scores_high() {
		...
		let diff = build_diff(&[("src/main.rs", &body_refs)]);
		let score = offload().estimate_bloat(&diff);
		assert!(score > 0.5, "whitespace-only hunk should score high, got {score}");
	}
	...
	fn apply_drops_whitespace_only_hunk() {
		...
		let r = offload()
			.apply(&diff, &CompressionContext::default(), &store)
			.expect("must compress");
		assert!(r.output.contains("[diff_noise: whitespace-only hunks dropped"));
		assert!(r.bytes_saved > 0);
	}
	...
	fn handles_diff_with_leading_commit_message() {
		...
		let lock: Vec<String> = (0..40).flat_map(|i| vec![format!("-old{i}"), format!("+new{i}")]).collect();
