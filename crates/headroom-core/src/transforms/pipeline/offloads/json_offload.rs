...
use crate::transforms::pipeline::traits::{CompressionContext, OffloadOutput, OffloadTransform, TransformError};
...
impl JsonOffload {
	...
	pub fn with_crusher_and_prose(
		...
		Self { crusher, config, prose: ProseFieldOffload::new(prose_config) }
	}
}
...
impl OffloadTransform for JsonOffload {
	...
	fn apply(
		...
		let result = self.crusher.crush_with_prose_hook(content, &ctx.query, 0.0, &prose_hook);
		if !result.was_modified {
			return Err(TransformError::skipped(NAME, "smart crusher returned passthrough"));
		}
...
fn count_row_separators(content: &str) -> usize {
	content.matches("},{").count() + content.matches("}, {").count() + content.matches("},\n").count()
}
...
mod tests {
	...
	fn build_tabular_array(n: usize) -> String {
		...
		for i in 0..n {
			...
			s.push_str(&format!("{{\"id\":{},\"name\":\"item-{}\",\"value\":{}}}", i, i, i * 100));
		}
		s.push(']');
		s
	}
	...
	fn estimate_bloat_scales_linearly_in_middle_range() {
		...
		let score = offload().estimate_bloat(&arr);
		assert!((0.4..=0.6).contains(&score), "expected mid-range score, got {score}");
	}
	...
	fn estimate_bloat_handles_whitespace_after_array_open() {
		...
		for i in 0..30 {
			...
			s.push_str(&format!("  {{\"id\":{i},\"name\":\"item-{i}\",\"value\":{}}}", i * 100));
		}
		s.push_str("\n]");
		let score = offload().estimate_bloat(&s);
		assert!(score > 0.4, "pretty-printed array should still score, got {score}");
	}
	...
	fn apply_propagates_query_anchors_into_smart_crusher() {
		...
		for i in 0..50 {
			...
			let name = if i == 17 { "needle".to_string() } else { format!("hay-{i}") };
			s.push_str(&format!("{{\"id\":{i},\"name\":\"{name}\",\"score\":{}}}", i % 7));
		}
		s.push(']');
		let store = InMemoryCcrStore::new();
		let ctx = CompressionContext::with_query("needle");
		let r = offload().apply(&s, &ctx, &store).expect("crusher should run");
		// Output should reference "needle" — SmartCrusher's anchor logic
		// should keep that row.
		assert!(r.output.contains("needle"), "anchor row should survive crush");
	}
	...
	fn cache_key_is_stable_across_calls_for_same_input() {
		...
		let r_a = offload().apply(&arr, &CompressionContext::default(), &store_a).expect("ok");
		let r_b = offload().apply(&arr, &CompressionContext::default(), &store_b).expect("ok");
		assert_eq!(
