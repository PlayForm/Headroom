...
impl DiffCompressor {
	...
	pub fn compress_with_store(
		...
		if original_line_count < self.config.min_lines_for_ccr {
			...
			return (pass_through_result(content, original_line_count), emit_span_and_return(stats));
		}
		...
		stats.context_lines_input = diff_files.iter().flat_map(|f| f.hunks.iter()).map(|h| h.context_lines).sum();
		...
		if diff_files.len() > self.config.max_files {
			...
			stats.files_dropped = dropped.iter().map(|f| format!("{} -> {}", f.old_file, f.new_file)).collect();
		}
		...
		for file in diff_files.iter() {
			...
			if let Some(orig) = &file.original_new_file_mode_line {
				if orig != "new file mode 100644" {
					stats.file_mode_normalizations.push((label.clone(), orig.clone()));
				}
			}
		...
		for file in diff_files {
			...
			compressed_files.push(DiffFile { hunks: compressed_hunks, ..file });
		}
		...
		if self.config.enable_ccr && (compressed_line_count as f64) < (original_line_count as f64) * savings_threshold {
...
fn is_diff_header(line: &str) -> bool {
	diff_git_regex().is_match(line) || diff_combined_regex().is_match(line) || diff_cc_regex().is_match(line)
}
...
fn parse_diff(lines: &[&str]) -> ParsedDiff {
	...
	ParsedDiff { pre_diff_lines, files, parse_warnings: warnings }
}
...
fn priority_patterns() -> &'static [Regex] {
	...
	RES.get_or_init(|| {
		vec![
			Regex::new(r"(?i)\b(error|exception|fail(?:ed|ure)?|fatal|critical|crash|panic)\b").unwrap(),
...
fn select_hunks(hunks: Vec<DiffHunk>, max_per_file: usize) -> (Vec<DiffHunk>, Vec<DiffHunk>) {
	...
	let last = if !indexed.is_empty() { Some(indexed.pop().unwrap()) } else { None };
	...
	middle_sorted.sort_by(|a, b| b.1.score.partial_cmp(&a.1.score).unwrap_or(std::cmp::Ordering::Equal));
	...
	let _ = n;
	(selected.into_iter().map(|(_, h)| h).collect(), dropped_middle)
}
...
fn reduce_context(hunk: &DiffHunk, max_context: usize) -> DiffHunk {
	...
	let change_positions: Vec<usize> = hunk
		...
		.iter()
		.enumerate()
		.filter_map(|(i, l)| if l.starts_with('+') || l.starts_with('-') { Some(i) } else { None })
		.collect();
...
mod tests {
	...
	fn max_hunks_per_file_cap_drops_excess_and_records_stats() {
		...
		let cfg = DiffCompressorConfig { max_hunks_per_file: 10, ..Default::default() };
	...
	fn max_files_cap_drops_files_and_records_names_in_stats() {
		...
		let cfg = DiffCompressorConfig { max_files: 20, ..Default::default() };
		...
		assert_eq!(stats.files_dropped.len(), 5, "expected 5 dropped file labels");
		// Each label should be the `old_file -> new_file` form.
		for label in &stats.files_dropped {
			assert!(label.contains("-> "), "label `{label}` should contain ` -> `");
		}
	}
	...
	fn binary_files_simplification_is_recorded() {
		...
		assert_eq!(stats.binary_files_simplified[0], "Binary files a/img.png and b/img.png differ");
	}
	...
	fn min_compression_ratio_for_ccr_is_configurable() {
		...
		let cfg = DiffCompressorConfig { min_compression_ratio_for_ccr: 0.5, ..Default::default() };
		let (r2, stats) = DiffCompressor::new(cfg).compress_with_stats(&build_synthetic_diff(8), "");
		assert!(
	...
	fn compress_with_store_no_op_when_ccr_skipped() {
		...
		let store = InMemoryCcrStore::new();
		let (r, _) = DiffCompressor::new(cfg).compress_with_store(&build_synthetic_diff(8), "", Some(&store));
		assert!(r.cache_key.is_none());
	...
	fn bugfix_rename_markers_are_preserved_in_output() {
		...
		let cfg = DiffCompressorConfig { min_lines_for_ccr: 5, ..Default::default() };
	...
	fn cjk_bigrams_from_query_runs() {
		...
		assert!(b.contains("数据") && b.contains("据库") && b.contains("库连") && b.contains("连接"));
		assert_eq!(b.len(), 4);
	...
	fn cjk_query_boosts_matching_hunk_into_kept_set() {
		...
		let input = "diff --git a/svc.py b/svc.py\n\
                     ...
                     -last_old\n\
                     +last_new\n";
		let mk = || DiffCompressorConfig { max_hunks_per_file: 3, min_lines_for_ccr: 5, ..Default::default() };
	...
	fn bugfix_combined_diff_3way_content_is_parsed_and_emitted() {
		...
		assert!(stats.files_total > 0, "parser found no files; combined-diff still broken");
	}
