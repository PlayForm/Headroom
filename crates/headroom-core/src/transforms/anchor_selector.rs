...
impl Default for AnchorConfig {
	fn default() -> Self {
		AnchorConfig {
			...
			historical_keywords: vec!["first", "oldest", "earliest", "original", "initial", "beginning"],
...
impl Default for AnchorWeights {
	fn default() -> Self {
		AnchorWeights { front: 0.5, middle: 0.1, back: 0.4 }
	}
}
...
impl AnchorWeights {
	...
	pub fn normalize(&self) -> AnchorWeights {
		...
		AnchorWeights { front: self.front / total, middle: self.middle / total, back: self.back / total }
	}
}
...
fn calculate_structural_uniqueness(item: &Value, all_items: &[Value]) -> f64 {
	let valid: Vec<&serde_json::Map<String, Value>> = all_items.iter().filter_map(|v| v.as_object()).collect();
	let n = valid.len();
	if n < 2 {
		return 0.5;
	}
	...
	let item_fields: HashSet<&String> = item.as_object().map(|o| o.keys().collect()).unwrap_or_default();
...
pub fn python_json_dumps_sort_keys(value: &Value) -> String {
	...
	write_python_json_inner(value, &mut out, JsonFmt { sort_keys: true, compact: false, ensure_ascii: true });
	out
}
...
pub fn python_json_dumps(value: &Value) -> String {
	...
	write_python_json_inner(
		value,
		&mut out,
		JsonFmt { sort_keys: false, compact: false, ensure_ascii: true },
	);
	out
}
...
pub fn python_safe_json_dumps(value: &Value) -> String {
	...
	write_python_json_inner(
		value,
		&mut out,
		JsonFmt { sort_keys: false, compact: true, ensure_ascii: false },
	);
	out
}
...
impl AnchorSelector {
	...
	pub fn base_weights_for_strategy(&self, strategy: AnchorStrategy) -> AnchorWeights {
		match strategy {
			...
			AnchorStrategy::Balanced => AnchorWeights { front: 0.45, middle: 0.1, back: 0.45 },
	...
	pub fn adjust_weights_for_query(&self, base: AnchorWeights, query: Option<&str>) -> AnchorWeights {
		...
		let has_recency = self.config.recency_keywords.iter().any(|kw| q_lower.contains(kw));
		let has_historical = self.config.historical_keywords.iter().any(|kw| q_lower.contains(kw));
	...
	pub fn select_anchors(
		...
		let back_start = array_size.saturating_sub(back_slots * 2).max((2 * array_size) / 3);
		let back_anchors = self.select_region(items, back_start, array_size, back_slots, &mut seen, false);
	...
	fn should_include(&self, items: &[Value], idx: usize, seen: &mut HashSet<String>, check_only: bool) -> bool {
...
mod tests {
	...
	fn json_dumps_nested_sort_keys_recursive() {
		...
		assert_eq!(python_json_dumps_sort_keys(&v), r#"{"outer": {"a": 2, "z": 1}}"#);
	}
	...
	fn json_dumps_null_bool() {
		...
		assert_eq!(python_json_dumps_sort_keys(&v), r#"{"a": null, "b": true, "c": false}"#);
	}
	...
	fn compute_item_hash_matches_python_basic() {
		...
		//   = "8aacdb17187e6acf"
		assert_eq!(compute_item_hash(&json!({"a": 1, "b": 2})), "8aacdb17187e6acf");
	}
	...
	fn weights_normalize_sums_to_one() {
		let w = AnchorWeights { front: 1.0, middle: 1.0, back: 2.0 }.normalize();
	...
	fn weights_normalize_zero_returns_default() {
		let w = AnchorWeights { front: 0.0, middle: 0.0, back: 0.0 }.normalize();
	...
	fn pattern_from_str_known_values() {
		assert_eq!(DataPattern::from_string("search_results"), DataPattern::SearchResults);
		assert_eq!(DataPattern::from_string("LOGS"), DataPattern::Logs);
		assert_eq!(DataPattern::from_string("time_series"), DataPattern::TimeSeries);
	}
	...
	fn budget_capped_by_array_size() {
		let c = AnchorConfig { min_anchor_slots: 50, ..AnchorConfig::default() };
	...
	fn strategy_mappings() {
		let s = selector();
		assert_eq!(s.strategy_for_pattern(DataPattern::SearchResults), AnchorStrategy::FrontHeavy);
		assert_eq!(s.strategy_for_pattern(DataPattern::Logs), AnchorStrategy::BackHeavy);
		assert_eq!(s.strategy_for_pattern(DataPattern::TimeSeries), AnchorStrategy::Balanced);
		assert_eq!(s.strategy_for_pattern(DataPattern::Generic), AnchorStrategy::Distributed);
	}
	...
	fn adjust_weights_recency_shifts_to_back() {
		let s = selector();
		let base = AnchorWeights { front: 0.5, middle: 0.1, back: 0.4 };
	...
	fn select_anchors_empty_returns_empty() {
		assert!(selector().select_anchors(&[], 10, DataPattern::Generic, None).is_empty());
	}
	...
	fn select_anchors_includes_first_and_last_for_distributed() {
		...
		assert!(max > 80, "last anchor should be near end (n=100), got {}", max);
	}
