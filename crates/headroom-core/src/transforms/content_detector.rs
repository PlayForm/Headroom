...
impl DetectionResult {
	fn new(content_type: ContentType, confidence: f64, metadata: Map<String, Value>) -> Self {
		Self { content_type, confidence, metadata }
	}
...
static HTML_DOCTYPE_PATTERN: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)^\s*<!doctype\s+html").unwrap());
...
static HTML_STRUCTURAL_TAGS: LazyLock<Regex> = LazyLock::new(|| {
	Regex::new(r"(?i)<(div|span|script|style|link|meta|nav|header|footer|aside|article|section|main)[\s>]").unwrap()
});
...
pub fn is_json_array_of_dicts(content: &str) -> bool {
	...
	result.metadata.get("is_dict_array").and_then(|v| v.as_bool()).unwrap_or(false)
}
...
fn try_detect_diff(content: &str) -> Option<DetectionResult> {
	...
	let confidence = (0.5 + (header_matches as f64) * 0.2 + (change_matches as f64) * 0.05).min(1.0);
	Some(DetectionResult::new(
		ContentType::GitDiff,
		confidence,
		json!({
