...
use icu_segmenter::{SentenceSegmenter, SentenceSegmenterBorrowed, WordSegmenter, WordSegmenterBorrowed};
...
impl TextCrusher {
	pub fn new(config: TextCrusherConfig) -> Self {
		TextCrusher { config, scorer: BM25Scorer::default() }
	}
	...
	pub fn compress(&self, content: &str, context: &str, target_ratio: Option<f64>) -> TextCrusherResult {
		let cfg = &self.config;
		...
		let relevance: Vec<f64> = if segments.iter().any(|s| s.chars().any(is_cjk)) {
			...
			self.scorer.score_batch(&seg_refs, context).iter().map(|r| r.score).collect()
		};
		...
		for i in 0..n {
			...
			let mut score = cfg.w_recency * recency + cfg.w_relevance * rel + cfg.w_salience * salience;
		...
		order.sort_by(|&a, &b| scores[b].partial_cmp(&scores[a]).unwrap_or(Ordering::Equal).then(a.cmp(&b)));
		...
		for &i in &order {
			...
			if !sh.is_empty() {
				let covered = sh.iter().filter(|s| seen.contains(*s)).count() as f64 / sh.len() as f64;
		...
		TextCrusherResult {
			compression_ratio: if orig_tok > 0 { comp_tok as f64 / orig_tok as f64 } else { 1.0 },
			compressed,
...
fn tokens(text: &str) -> Vec<String> {
	if text.chars().any(is_cjk) { tokens_icu(text) } else { tokens_ascii(text) }
}
...
mod tests {
	...
	fn cjk_splits_on_full_width_terminators() {
		...
		assert!(segs.len() >= 4, "expected multiple CJK sentences, got {segs:?}");
		for s in &segs {
	...
	fn korean_tokenizes_and_splits() {
		...
		assert!(tokens(ko).len() >= 4, "Korean must yield multiple tokens via ICU");
	...
	fn japanese_no_space_tokenizes_via_dictionary() {
		...
		assert!(tokens(ja).len() >= 5, "Japanese must split into multiple ICU tokens");
		assert!(split_segments(ja).len() >= 2, "Japanese sentences must split on 。");
	}
	...
	fn cjk_token_count_and_ratio_are_sane() {
		...
		assert!(r.original_tokens > 10, "CJK token count must not collapse to 1");
		assert!(r.compressed_tokens < r.original_tokens);
