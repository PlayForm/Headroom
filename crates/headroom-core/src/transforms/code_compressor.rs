...
fn symbol_in_context(name_lower: &str, words: &BTreeSet<String>, context_lower: &str, has_cjk: bool) -> bool {
	...
	pub fn patterns() -> &'static [(CodeLanguage, Vec<Regex>)] {
		...
		P.get_or_init(|| {
			...
			vec![
				...
				(
					CodeLanguage::Rust,
					vec![c(r"(?m)^\s*(fn|struct|enum|impl|mod|use|pub)\s+"), c(r"(?m)^\s*#\[")],
				),
				(
					CodeLanguage::Java,
					vec![
				...
				(
					CodeLanguage::Cpp,
					vec![c(r#"(?m)^\s*#include\s*[<"]"#), c(r"(?m)\bnamespace\s+\w+"), c(r"(?m)::\w+")],
				),
			]
		})
	}
...
pub fn detect_language(code: &str) -> (CodeLanguage, f64) {
	...
	let get = |cs: &[(CodeLanguage, i64)], l: CodeLanguage| cs.iter().find(|(x, _)| *x == l).map(|(_, s)| *s);
	...
	if let (Some(cpp), Some(_c)) = (get(&candidates, CodeLanguage::Cpp), get(&candidates, CodeLanguage::C)) {
		if cpp >= 2 {
...
impl CodeAwareCompressor {
	...
	pub fn compress_with(&self, code: &str, language: Option<&str>, context: &str) -> CodeCompressionResult {
		if code.trim().is_empty() {
		...
		let Some((compressed, structure, symbol_scores)) = self.compress_with_ast(code, detected_lang, context) else {
	...
	fn compress_with_ast(&self, code: &str, language: CodeLanguage, context: &str) -> Option<AstCompression> {
...
fn extract_generic_structure(code: &str) -> CodeStructure {
	CodeStructure { other: code.split('\n').map(|s| s.to_string()).collect(), ..Default::default() }
}
...
impl CodeAwareCompressor {
	...
	fn analyze_symbol_importance(
		...
		collect_definitions(root, "", code, &is_def, lang.decorator_node, &mut definitions, &mut bare_names);
		...
		for (qname, _) in &definitions {
			...
			if language == CodeLanguage::Python && short.starts_with("__") && short.ends_with("__") {
				raw += 2.0;
			} else if language == CodeLanguage::Go && short.chars().next().is_some_and(|c| c.is_uppercase()) {
				raw += 1.0;
			}

			// Context boost: the relevance query named this symbol.
			if symbol_in_context(&short.to_lowercase(), &context_words, &context_lower, context_has_cjk) {
				raw += 3.0;
			}
		...
		SymbolAnalysis { scores, calls: function_calls, bare_names, body_line_counts }
	}
...
impl<'a> Ctx<'a> {
	...
	fn compress_function_ast(&self, node: Node) -> String {
		...
		let body_limit = get_body_limit(func_name.as_deref(), self.body_limits, self.config.max_body_lines);
				...
				match self.config.docstring_mode {
					...
					DocstringMode::FirstLine => {
						if ds_lines_count == 1 {
							...
							docstring_text = first_line_docstring(first_ds_line, &body_lines, ds_start_rel);
						}
					},
		...
		const SKIP_TYPES: &[&str] = &["{", "}", ";", ",", "comment", "line_comment", "block_comment"];
		...
		let total_body_lines_count: i64 = body_stmts.iter().map(|(s, e)| (*e - *s + 1) as i64).sum();
	...
	fn compress_class_ast(&self, node: Node) -> String {
		...
		let header_lines: Vec<&str> = if sig_end > 0 { node_lines[..sig_end].to_vec() } else { vec![node_lines[0]] };
...
mod tests {
	...
	fn cjk_query_isolates_wrapped_ascii_symbol() {
		...
		let (words, lowered, has_cjk) = query_context_tokens("请重点保留（parse_config）的解析配置");
		assert!(has_cjk);
