...
impl StackTraceDetector {
	...
	fn is_java_at_frame(s: &str) -> bool {
		...
		body.chars().all(|c| c.is_ascii_alphanumeric() || matches!(c, '.' | '_' | '$')) && !body.is_empty()
	}
	...
	fn terminates(flavor: TraceFlavor, line: &str) -> bool {
		...
		match flavor {
			...
			TraceFlavor::Go => !trimmed.chars().next().is_some_and(|c| c.is_ascii_digit()) && !line.is_empty(),
		}
	}
}
	...
	pub fn parse_lines(&self, lines: &[&str]) -> Vec<LogLine> {
		...
		for (i, line) in lines.iter().enumerate() {
			...
			if let Some(flavor) = active {
				if trace_lines >= self.config.stack_trace_max_lines || StackTraceDetector::terminates(flavor, line) {
					active = None;
					trace_lines = 0;
