...
impl LogCompressionResult {
	...
	pub fn lines_omitted(&self) -> usize {
		self.original_line_count.saturating_sub(self.compressed_line_count)
	}
}
...
impl FormatDetector {
	fn new() -> Self {
		let table: &[(LogFormat, &[&str])] = &[
			...
			(LogFormat::Npm, &["npm ERR!", "npm WARN", "npm info", "npm http"]),
			(
				LogFormat::Cargo,
				&["Compiling ", "Finished ", "Running ", "warning: ", "error[E"],
			),
			(LogFormat::Jest, &["PASS ", "FAIL ", "Test Suites:"]),
			(LogFormat::Make, &["make[", "make:", "gcc ", "g++ ", "clang "]),
		];
...
impl LevelClassifier {
	fn new() -> Self {
		...
		let entries: &[(LogLevel, &[&str])] = &[
			...
			(LogLevel::Fail, &["FAIL", "FAILED", "fail", "failed", "Fail", "Failed"]),
			(LogLevel::Warn, &["WARN", "WARNING", "warn", "warning", "Warn", "Warning"]),
...
impl StackTraceDetector {
	fn flavor_for(line: &str) -> Option<TraceFlavor> {
		...
		if trimmed.starts_with("Traceback (most recent call last)") || Self::is_python_file_frame(trimmed) {
	...
	fn is_python_file_frame(s: &str) -> bool {
		...
		s.starts_with("File \"") && s.contains("\", line ") && s.bytes().next_back().is_some_and(|b| b.is_ascii_digit())
	}
	...
	fn has_line_col_suffix(s: &str) -> bool {
		...
		for i in 0..bytes.len().saturating_sub(2) {
			if bytes[i] == b':' && bytes[i + 1].is_ascii_digit() {
				...
				if j < bytes.len()
					&& bytes[j] == b':'
					&& bytes.get(j + 1).copied().map(|b| b.is_ascii_digit()).unwrap_or(false)
				{
					return true;
				}
			}
	...
	fn is_rust_backtrace_frame(s: &str) -> bool {
		...
		rest.starts_with("0x") && rest[2..].chars().take_while(|c| c.is_ascii_hexdigit()).count() > 0
	}
...
fn is_runtime_frame(line: &str) -> bool {
	let trimmed = line.trim_start();
	RUNTIME_FRAME_PREFIXES.iter().any(|p| trimmed.starts_with(p))
...
/// with it.
fn collapse_trace_frames(stack: &[LogLine], head_frames: usize, app_frames: usize) -> CollapsedTrace {
	...
	for line in stack {
		...
		} else if prev_dropped && line.content.starts_with([' ', '\t']) && !is_chain_head_line(&line.content) {
	...
	flush_run(&mut kept, &mut run_start, &mut run_len);
	CollapsedTrace { kept, dropped_indices }
}
...
fn is_summary_line(line: &str) -> bool {
	...
	for prefix in &["Test ", "Tests ", "Tests:", "Test:", "Suite ", "Suites ", "Suites:", "Suite:"] {
		if let Some(rest) = line.strip_prefix(prefix) {
			// Need digits somewhere after the prefix.
			return rest.chars().find(|c| !c.is_whitespace()).is_some_and(|c| c.is_ascii_digit());
		}
	}
...
impl LogCompressor {
	pub fn new(config: LogCompressorConfig) -> Self {
		Self { config, formats: FormatDetector::new(), levels: LevelClassifier::new() }
	}
	...
	pub fn select_lines(&self, log_lines: &[LogLine], bias: f64, stats: &mut LogCompressorStats) -> Vec<LogLine> {
		let all_strings: Vec<&str> = log_lines.iter().map(|l| l.content.as_str()).collect();
		let adaptive_max = compute_optimal_k(&all_strings, bias, 10, Some(self.config.max_total_lines));
		...
		for stack in stack_traces.iter().take(self.config.max_stack_traces) {
			...
			if self.config.collapse_runtime_frames && stack.len() > self.config.stack_trace_max_lines {
				let collapsed =
					collapse_trace_frames(stack, self.config.trace_head_frames, self.config.trace_app_frames);
				stats.runtime_frames_collapsed += collapsed.dropped_indices.len();
				collapsed_frame_indices.extend(collapsed.dropped_indices);
				for line in collapsed.kept.into_iter().take(self.config.stack_trace_max_lines) {
					selected.insert(line);
				}
			} else {
		...
		for idx in context_indices {
			...
			if !selected_indices.contains(&idx) && idx < log_lines.len() && !collapsed_frame_indices.contains(&idx) {
	...
	pub fn format_output(&self, selected: &[LogLine], all_lines: &[LogLine]) -> (String, BTreeMap<String, u64>) {
		...
		if omitted > 0 {
			...
			for (label, key) in [("ERROR", "errors"), ("FAIL", "fails"), ("WARN", "warnings"), ("INFO", "info")] {
			...
			if !summary_parts.is_empty() {
				output.push(format!("[{} lines omitted: {}]", omitted, summary_parts.join(", ")));
			}
		}
		(output.join("\n"), stats)
	}
...
mod tests {
	...
	fn trace_flags(c: &LogCompressor, lines: &[&str]) -> Vec<bool> {
		c.parse_lines(lines).iter().map(|l| l.is_stack_trace).collect()
	}
	...
	fn java_chained_trace(runtime_frames: usize) -> String {
		let mut lines = vec!["Exception in thread \"main\" java.lang.IllegalStateException: boom".to_string()];
		...
		for i in 0..runtime_frames {
			lines.push(format!("at java.base/java.util.stream.Op{}.eval(Op{}.java:{})", i, i, i + 1));
		}
		lines.push("Caused by: java.io.IOException: disk gone".into());
		lines.push("at com.example.Disk.read(Disk.java:77)".into());
		for i in 0..runtime_frames {
			lines.push(format!("at java.base/java.lang.Thread{}.run(Thread.java:{})", i, i + 1));
		}
