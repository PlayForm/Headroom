...
impl SearchMatch {
	pub fn new(file: impl Into<String>, line_number: u64, content: impl Into<String>) -> Self {
		Self { file: file.into(), line_number, content: content.into(), score: 0.0 }
	}
}
...
impl FileMatches {
	pub fn new(file: impl Into<String>) -> Self {
		Self { file: file.into(), matches: Vec::new() }
	}
...
impl SearchCompressionResult {
	...
	pub fn matches_omitted(&self) -> usize {
		self.original_match_count.saturating_sub(self.compressed_match_count)
	}
}
...
impl SearchCompressor {
	pub fn new(config: SearchCompressorConfig) -> Self {
		Self { config, importance: Box::new(crate::signals::KeywordDetector::new()) }
	}
	...
	pub fn with_detector<D: LineImportanceDetector + 'static>(config: SearchCompressorConfig, detector: D) -> Self {
		Self { config, importance: Box::new(detector) }
	}
	...
	pub fn select_matches(
		...
		let adaptive_total = compute_optimal_k(&all_refs, bias, 5, Some(self.config.max_total_matches));
		...
		for (file, fm) in by_score {
			...
			let push_unique =
				|m: &SearchMatch, file_selected: &mut Vec<SearchMatch>, seen: &mut BTreeSet<(u64, u64)>| {
			...
			selected.insert(file.clone(), FileMatches { file: file.clone(), matches: file_selected });
		}
...
fn has_extension_dot(tok: &str) -> bool {
	...
	b.iter().enumerate().any(|&(i, &c)| {
		...
		let tail = &b[i + 1..];
		let end = tail.iter().position(|c| !c.is_ascii_alphanumeric()).unwrap_or(tail.len());
		let ext = &tail[..end];
...
mod tests {
	...
	fn parses_context_line_for_extensionless_path() {
		...
		for (line, file, no, body) in [
			...
			("LICENSE-1-Copyright-2026-Acme", "LICENSE", 1, "Copyright-2026-Acme"),
			("Makefile-7-build-2-stage", "Makefile", 7, "build-2-stage"),
			("Dockerfile-3-FROM-18-alpine", "Dockerfile", 3, "FROM-18-alpine"),
			("bin/run-3-exec-9-now", "bin/run", 3, "exec-9-now"),
			(".github/workflows/ci-9-2026-01-02", ".github/workflows/ci", 9, "2026-01-02"),
		] {
			assert_eq!(
				parse_line(line),
	...
	fn fixed_in_3e2_handles_dashes_in_filename_with_ripgrep_context() {
		...
		assert_eq!(
			...
			Some(("pre-commit-config.yaml".into(), 42, "fail_fast: true".into()))
		);
	}
	...
	fn date_stamped_path_is_not_misread_as_line_number_marker() {
		...
		assert_eq!(
			...
			Some(("logs/2026-11-23/app.log".into(), 11, "context before".into()))
		);
		...
		assert_eq!(
			...
			Some(("migrations/20240101-002-add_users.sql".into(), 9, "  id BIGINT".into()))
		);
	}
...
logs/2026-05-04/app.log:7:ERROR bang";
		...
		assert_eq!(files, vec!["logs/2026-05-03/app.log", "logs/2026-05-04/app.log"]);
		assert_eq!(parsed["logs/2026-05-03/app.log"].matches[0].line_number, 12);
		assert_eq!(parsed["logs/2026-05-03/app.log"].matches[0].content, "ERROR boom");
	}
...
src/main.py-44-context line";
	...
	fn scoring_boosts_error_lines_in_search_context() {
		...
		fm.matches.push(SearchMatch::new("src/auth.py", 10, "ERROR auth failed"));
		fm.matches.push(SearchMatch::new("src/auth.py", 11, "plain auth line"));
	...
	fn select_respects_per_file_cap_and_global_cap() {
		...
		for (file, n) in [("a.py", 5), ("b.py", 4), ("c.py", 3)] {
			...
			for i in 0..n {
				fm.matches.push(SearchMatch::new(file, i + 1, format!("line {}", i + 1)));
			}
		...
		for fm in selected.values() {
			...
			assert!(fm.matches.windows(2).all(|w| w[0].line_number < w[1].line_number));
		}
	}
	...
	fn ccr_skipped_when_below_min_matches() {
		let compressor =
			SearchCompressor::new(SearchCompressorConfig { min_matches_for_ccr: 100, ..Default::default() });
	...
	fn ccr_skipped_when_disabled() {
		let compressor = SearchCompressor::new(SearchCompressorConfig { enable_ccr: false, ..Default::default() });
		let mut content = String::new();
		for i in 1..=20 {
