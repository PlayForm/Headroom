...
pub(crate) fn magika_runtime_available_for_session_init() -> Result<(), String> {
	if !magika_onnx_runtime_supported_by_cpu() {
		return Err("Magika ONNX Runtime backend requires AVX2 on this platform; \
...
fn discover_onnxruntime_libraries() -> Vec<PathBuf> {
	...
	if let Some(user_profile) = env_path("USERPROFILE") {
		roots.extend(versioned_children(
			user_profile.join(".pyenv").join("pyenv-win").join("versions"),
		));
		roots.extend(versioned_children(
			user_profile.join("AppData").join("Local").join("Programs").join("Python"),
		));
		roots.extend(versioned_children(user_profile.join("AppData").join("Roaming").join("Python")));
	}
...
#[cfg(not(any(target_os = "windows", all(target_os = "macos", target_arch = "x86_64"))))]
fn dynamic_ort_loader_ready() -> Result<(), String> {
	Ok(())
}
		...
		if let Err(init_reason) = magika_runtime_available_for_session_init() {
			...
			match magika_detect(content) {
				Err(MagikaDetectorError::Init(msg)) => {
					assert!(msg == init_reason, "{hint}: expected init error {init_reason:?}, got: {msg:?}");
				},
