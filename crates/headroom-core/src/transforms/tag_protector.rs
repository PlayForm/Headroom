...
fn pick_placeholder_prefix(text: &str) -> (String, bool) {
	...
	let prefix = FALLBACK.get_or_init(|| "{{HEADROOM_TAG_FALLBACK_a4f1c7e2_".to_string()).clone();
	(prefix, true)
}
...
enum TagParse {
	...
	Open { name_end: usize, tag_end: usize, is_self_closing: bool },
	/// Closing tag (`</name>`).
...
fn parse_tag_at(bytes: &[u8], start: usize) -> TagParse {
	...
	if is_close {
		...
		return TagParse::Close { name_end, tag_end: i + 1 };
	}
	...
	while i < n {
		match bytes[i] {
			b'>' => {
				return TagParse::Open { name_end, tag_end: i + 1, is_self_closing: self_closing };
			},
			b'/' => {
				self_closing = true;
				i += 1;
...
pub fn protect_tags(text: &str, compress_tagged_content: bool) -> (String, Vec<(String, String)>, ProtectStats) {
...
fn identify_spans(text: &str, compress_tagged_content: bool, stats: &mut ProtectStats) -> Vec<Span> {
	...
	while i < n {
		...
		match parse_tag_at(bytes, i) {
			...
			TagParse::Open { name_end, tag_end, is_self_closing } => {
				stats.tags_seen += 1;
				...
				if is_self_closing {
					spans.push(Span { start: i, end: tag_end, kind: SpanKind::SelfClosing });
				...
				if compress_tagged_content {
					...
					spans.push(Span { start: i, end: tag_end, kind: SpanKind::OpenMarker });
				}
				// Both modes push to the stack so close-matching works.
				stack.push(OpenTag { name_lower: name.to_ascii_lowercase(), open_start: i });
				i = tag_end;
			},
			TagParse::Close { name_end, tag_end } => {
				...
				let matching = stack.iter().rposition(|open| open.name_lower == close_name_lower);
				...
				match matching {
					Some(stack_idx) => {
						if compress_tagged_content {
							...
							let _ = stack.pop();
							spans.push(Span { start: i, end: tag_end, kind: SpanKind::CloseMarker });
						} else {
							...
							spans.push(Span { start: open_start, end: tag_end, kind: SpanKind::Block });
...
fn emit_output(text: &str, spans: &[Span], prefix: &str) -> Option<(String, Vec<(String, String)>)> {
...
pub fn restore_tags_with_request_id(text: &str, blocks: &[(String, String)], request_id: Option<&str>) -> String {
	if blocks.is_empty() {
...
mod tests {
	...
	fn restore_lost_placeholder_discards_wrap() {
		...
		let blocks = vec![("{{HEADROOM_TAG_0}}".to_string(), "<tag>data</tag>".to_string())];
	...
	fn restore_partial_loss_keeps_present_drops_lost() {
		...
		let blocks = vec![
			("{{HEADROOM_TAG_0}}".to_string(), "<a>1</a>".to_string()),
			("{{HEADROOM_TAG_1}}".to_string(), "<lost>x</lost>".to_string()),
		];
