...
impl std::fmt::Debug for Kompress {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		f.debug_struct("Kompress").field("config", &self.config).finish_non_exhaustive()
	}
}
...
impl Kompress {
	...
	pub fn from_pretrained(config: KompressConfig) -> Result<Self, KompressError> {
		let api = hf_hub::api::sync::Api::new()
			.map_err(|e| KompressError::Hub { repo: config.model_id.clone(), source: Box::new(e) })?;

		let tok_path = api
			.model(config.tokenizer_repo.clone())
			.get("tokenizer.json")
			.map_err(|e| KompressError::Hub { repo: config.tokenizer_repo.clone(), source: Box::new(e) })?;
	...
	fn compress_inner(&self, content: &str, target_ratio: Option<f64>) -> KompressResult {
		...
		let compressed_words: Vec<&str> = kept_ids.iter().filter(|&&w| w < n_words).map(|&w| words[w]).collect();
		let compressed_tokens = compressed_words.len();
		let compressed = compressed_words.join(" ");
		let compression_ratio = if n_words == 0 { 1.0 } else { compressed_tokens as f64 / n_words as f64 };
		KompressResult {
			compressed,
	...
	fn score_chunk(
		...
		let encoding = self.tokenizer.encode(EncodeInput::Single(InputSequence::from(seq_in)), true)?;
...
fn load_tokenizer(path: &Path, repo: &str) -> Result<Tokenizer, KompressError> {
	let mut tokenizer =
		Tokenizer::from_file(path).map_err(|e| KompressError::Tokenizer { repo: repo.to_string(), source: e })?;
	...
	tokenizer
		...
		.map_err(|e| KompressError::Tokenizer { repo: repo.to_string(), source: e })?;
	Ok(tokenizer)
}
