...
use super::line_importance::{ImportanceCategory, ImportanceContext, ImportanceSignal, LineImportanceDetector};
...
impl KeywordRegistry {
	...
	pub fn default_set() -> Self {
		Self {
			...
			importance: vec!["important", "note", "todo", "fixme", "hack", "xxx", "bug", "fix"],
			security: vec!["security", "auth", "password", "secret"],
			markdown_prefixes: vec!["# ", "## ", "### ", "#### ", "**", "> "],
			error_indicators: vec!["error", "fail", "exception", "traceback", "fatal", "panic", "crash"],
		}
	}
...
impl KeywordDetector {
	...
	pub fn with_registry(registry: KeywordRegistry) -> Self {
		...
		let security = CategoryAutomaton::build(&[(ImportanceCategory::Security, &registry.security)]);
		let indicators = AhoCorasickBuilder::new()
			.ascii_case_insensitive(true)
			.match_kind(MatchKind::LeftmostLongest)
			.build(&registry.error_indicators)
			.expect("indicator automaton must build (static input)");
		Self { registry, universal, warning, security, indicators }
	}
	...
	fn match_in_context(&self, line: &str, ctx: ImportanceContext) -> Option<(ImportanceCategory, f32)> {
		...
		if matches!(ctx, ImportanceContext::Text) {
			if let Some(prefix) = self.registry.markdown_prefixes.iter().find(|p| line.starts_with(*p)) {
				let _ = prefix;
...
impl LineImportanceDetector for KeywordDetector {
	fn score(&self, line: &str, ctx: ImportanceContext) -> ImportanceSignal {
		match self.match_in_context(line, ctx) {
			Some((category, priority)) => ImportanceSignal::matched(category, priority, KEYWORD_CONFIDENCE),
...
mod tests {
	...
	fn warning_fires_in_search_but_not_diff() {
		...
		// preserve that.
		let in_diff = detect("warning: deprecated API alone with no errors", ImportanceContext::Diff);
