...
impl HfTokenizer {
	...
	pub fn from_bytes(name: impl Into<String>, bytes: &[u8]) -> Result<Self, HfTokenizerError> {
		let name = name.into();
		let inner = HfInner::from_bytes(bytes).map_err(|e| HfTokenizerError::Load { name: name.clone(), source: e })?;
		Ok(Self { name, inner: Arc::new(inner) })
	}

	/// Build from a `tokenizer.json` on disk.
	pub fn from_file(name: impl Into<String>, path: impl AsRef<Path>) -> Result<Self, HfTokenizerError> {
		let name = name.into();
		let inner =
			HfInner::from_file(path.as_ref()).map_err(|e| HfTokenizerError::Load { name: name.clone(), source: e })?;
		Ok(Self { name, inner: Arc::new(inner) })
	}
	...
	pub fn from_pretrained(repo: &str) -> Result<Self, HfTokenizerError> {
		let api = hf_hub::api::sync::Api::new()
			.map_err(|e| HfTokenizerError::Hub { repo: repo.to_string(), source: Box::new(e) })?;
		let path = api
...
mod tests {
	...
	fn tiny() -> HfTokenizer {
		HfTokenizer::from_bytes("tiny-test", TINY_TOKENIZER_JSON.as_bytes()).expect("tiny tokenizer.json parses")
	}
