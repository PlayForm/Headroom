...
/// and panic.
static O200K: LazyLock<Arc<CoreBPE>> = LazyLock::new(|| Arc::new(tiktoken_rs::o200k_base().expect("o200k_base init")));
static CL100K: LazyLock<Arc<CoreBPE>> =
	LazyLock::new(|| Arc::new(tiktoken_rs::cl100k_base().expect("cl100k_base init")));
static P50K: LazyLock<Arc<CoreBPE>> = LazyLock::new(|| Arc::new(tiktoken_rs::p50k_base().expect("p50k_base init")));
static R50K: LazyLock<Arc<CoreBPE>> = LazyLock::new(|| Arc::new(tiktoken_rs::r50k_base().expect("r50k_base init")));
...
impl TiktokenCounter {
	...
	pub fn for_model(model: &str) -> Result<Self, TiktokenError> {
		...
		Ok(Self { model: model.to_string(), encoding_name, bpe })
	}
...
fn encoding_for(model: &str) -> Result<&'static str, TiktokenError> {
	...
	if m.starts_with("code-") || m.starts_with("text-davinci-002") || m.starts_with("text-davinci-003") {
		return Ok("p50k_base");
	}
...
mod tests {
	...
	fn known_token_counts_for_o200k() {
		...
		assert_eq!(t.count_text("the quick brown fox jumps over the lazy dog"), 9);
	}
	...
	fn encoding_dispatch() {
		for (model, expected) in [
			...
			let t = TiktokenCounter::for_model(model).unwrap_or_else(|e| panic!("for_model({model}) failed: {e}"));
