...
fn force_lossy_config() -> SmartCrusherConfig {
	SmartCrusherConfig { lossless_min_savings_ratio: 0.99, ..SmartCrusherConfig::default() }
}
...
fn default_crusher_stores_dropped_rows() {
	...
	let hash = result.ccr_hash.as_ref().expect("lossy path must emit a hash");
...
fn lossy_crush_injects_marker_into_output_json() {
	...
	let marker_hash = extract_hash_from_marker(&result.compressed).expect("marker must embed a hash");
...
fn nested_array_inside_object_gets_marker_injected() {
	...
	assert!(marker.starts_with("<<ccr:"), "expected CCR marker text, got: {marker}");
}
...
fn opaque_string_in_object_emits_marker_and_stores_original() {
	...
	assert_eq!(store.len(), starting_len + 1, "opaque-string CCR must write to store");
...
fn stringified_json_array_recurses_and_compacts() {
	...
	let crusher = SmartCrusher::new(force_lossy_config());
	let inner = (0..50).map(|i| json!({"id": i, "status": "ok"})).collect::<Vec<_>>();
...
fn nested_structured_prose_leaf_uses_ccr() {
	...
	assert!(result.steps_applied.iter().any(|step| step == "json_offload"));
