...
fn dispatch(body: &[u8]) -> LiveZoneOutcome {
	compress_anthropic_live_zone(body, 0, AuthMode::Payg, DEFAULT_MODEL).expect("dispatcher returns Ok on valid bodies")
}
...
fn find_byte_range(haystack: &[u8], needle: &[u8]) -> (usize, usize) {
	let pos = haystack.windows(needle.len()).position(|w| w == needle).unwrap_or_else(|| {
		panic!(
...
fn json_tool_result_routes_to_smart_crusher() {
	...
	let manifest = match &out {
		...
		LiveZoneOutcome::NoChange { manifest } => {
			panic!("expected SmartCrusher to compress 200 homogeneous dicts; got NoChange. manifest: {manifest:?}")
		},
	};
	let action = manifest
		.block_outcomes
		...
		.clone();
	match action {
		BlockAction::Compressed { strategy, original_bytes, compressed_bytes, original_tokens, compressed_tokens } => {
...
fn log_tool_result_routes_to_log_compressor() {
	...
	let action = manifest
		...
		.clone();
	match action {
		BlockAction::Compressed { strategy, original_bytes, compressed_bytes, .. } => {
...
fn byte_fidelity_outside_compressed_block() {
	...
	let new_body = match &out {
		...
		LiveZoneOutcome::NoChange { manifest } => {
			panic!("expected Modified outcome on 50 KB SmartCrusher fodder; got NoChange. manifest: {manifest:?}")
		},
	};
