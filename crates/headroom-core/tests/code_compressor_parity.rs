...
use headroom_core::transforms::code_compressor::{CodeAwareCompressor, CodeCompressorConfig, DocstringMode};
...
fn config_from_fixture(config: &Value) -> CodeCompressorConfig {
	...
	CodeCompressorConfig {
		...
		language_hint: config.get("language_hint").and_then(|v| v.as_str()).map(|s| s.to_string()),
		...
		enable_ccr: config.get("enable_ccr").and_then(|v| v.as_bool()).unwrap_or(d.enable_ccr),
		ccr_ttl: config.get("ccr_ttl").and_then(|v| v.as_i64()).unwrap_or(d.ccr_ttl),
	}
}

#[test]
fn code_compressor_matches_python_fixtures_byte_for_byte() {
	let fixtures_dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../tests/parity/fixtures/code_aware_compressor");
	assert!(
		fixtures_dir.exists(),
