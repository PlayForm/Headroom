...
fn kompress_matches_python_fixtures_byte_for_byte() {
	...
	let onnx = hf_cache_file("models--chopratejas--kompress-v2-base", &["onnx", "kompress-int8-wo.onnx"]);
	let (tok, onnx) = match (tok, onnx) {
		(Some(t), Some(o)) => (t, o),
		_ => {
			eprintln!(
				"SKIP: kompress model/tokenizer not in HF cache; run `python scripts/record_kompress_trace.py` first"
			);
			return;
		},
	};

	let fixtures_dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../tests/parity/fixtures/kompress");
	...
	for path in paths {
		...
		let exp_ratio = out["compression_ratio"].as_f64().expect("output.compression_ratio");
