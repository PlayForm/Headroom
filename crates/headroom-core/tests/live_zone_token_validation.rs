...
fn compressed_more_tokens_falls_back() {
	...
	for i in 0..40 {
		...
		let blob_clean: String = blob.chars().map(|c| if c.is_ascii_alphanumeric() { c } else { 'A' }).collect();
	...
	match action {
		BlockAction::RejectedNotSmaller { original_tokens, compressed_tokens, .. } => {
			assert!(
...
fn compressed_fewer_tokens_accepted() {
	...
	match action {
		BlockAction::Compressed { strategy, original_tokens, compressed_tokens, .. } => {
		...
		other => panic!("expected token-shrinking Compressed for 200-dict SmartCrusher fodder, got {other:?}"),
	}
}
