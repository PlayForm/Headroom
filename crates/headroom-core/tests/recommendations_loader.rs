...
use headroom_core::transforms::recommendations::{AuthMode, RecommendationStore, RecommendationsError};
...
fn tempdir() -> TempDir {
	...
	let p = std::env::temp_dir().join(format!("headroom-recommendations-{nanos}-{:?}", std::thread::current().id()));
...
fn loads_valid_toml() {
	...
	let oauth = store.lookup(AuthMode::OAuth, "gpt-4o", "cafebabe44556677").expect("oauth row");
