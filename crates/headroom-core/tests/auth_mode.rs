...
fn non_utf8_user_agent_does_not_panic() {
	...
	headers.insert("user-agent", HeaderValue::from_bytes(b"\xFF\xFE\xFFinject\x80").unwrap());
...
fn both_non_utf8_ua_and_auth_do_not_panic() {
	...
	// default is Payg.
	let mut headers = HeaderMap::new();
	headers.insert("user-agent", HeaderValue::from_bytes(b"\xFFuser\xFFagent").unwrap());
	headers.insert("authorization", HeaderValue::from_bytes(b"Bearer \x80\x81\x82").unwrap());
	assert_eq!(classify(&headers), AuthMode::Payg);
}
...
fn bearer_token_and_x_api_key_both_present_authorization_wins() {
	...
	let h = headers(&[("authorization", "Bearer sk-ant...beef"), ("x-api-key", "sk-ant...7890")]);
	assert_eq!(classify(&h), AuthMode::Payg);
}
...
fn ua_substring_not_a_prefix_does_not_match() {
	...
	let h = headers(&[("user-agent", "Claude Code/2.0"), ("authorization", "Bearer sk-ant...y123")]);
	assert_eq!(classify(&h), AuthMode::Payg);
}
