...
use headroom_core::transforms::live_zone::{
	compress_anthropic_live_zone, compress_anthropic_live_zone_with_ccr, AuthMode, LiveZoneOutcome, DEFAULT_MODEL,
};
...
fn ccr_marker_injected_when_store_wired() {
	...
	let outcome = compress_anthropic_live_zone_with_ccr(&body, 0, AuthMode::Payg, DEFAULT_MODEL, Some(&store))
...
fn no_marker_when_store_omitted() {
	...
	let outcome = compress_anthropic_live_zone(&body, 0, AuthMode::Payg, DEFAULT_MODEL).expect("dispatcher");
...
fn store_only_populated_after_token_gate_admits() {
	...
	let store = InMemoryCcrStore::new();
	let _ = compress_anthropic_live_zone_with_ccr(&body, 0, AuthMode::Payg, DEFAULT_MODEL, Some(&store))
		.expect("dispatcher");
