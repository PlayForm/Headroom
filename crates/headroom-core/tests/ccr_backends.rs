...
use headroom_core::ccr::backends::{from_config, CcrBackendConfig, InMemoryCcrStore, SqliteCcrStore};
...
fn from_config_sqlite_roundtrip() {
	let dir = tempfile::tempdir().expect("tempdir");
	let path = dir.path().join("ccr.sqlite");
	let cfg = CcrBackendConfig::Sqlite { path: path.clone(), ttl_seconds: 300 };
...
fn from_config_redis_unsupported_when_feature_off() {
	...
	let cfg = CcrBackendConfig::Redis { url: "redis://127.0.0.1:6379".to_string(), ttl_seconds: 300, key_prefix: None };
...
fn backend_swap_byte_equal_keys() {
	...
	for payload in &payloads {
		...
		assert_eq!(v_sqlite, v_mem, "sqlite and in-memory must return byte-equal payloads");
	}
}
...
fn in_memory_get_refreshes_idle_ttl() {
	...
	assert_eq!(store.get(&hash), None, "an entry idle past its window must expire");
}
...
fn in_memory_max_lifetime_caps_sliding_window() {
	...
	assert!(expired, "constant access must not extend an entry past its max lifetime");
}
...
fn sqlite_get_refreshes_idle_ttl() {
	...
	assert_eq!(store.get(&hash), None, "an entry idle past its window must be purged");
}
...
fn sqlite_max_lifetime_caps_sliding_window() {
	...
	let store = SqliteCcrStore::open_with_ttls(&path, 2, 3).expect("open sqlite store with ceiling");
...
mod redis_tests {
	...
	fn redis_round_trip_via_from_config() {
		...
		let cfg = CcrBackendConfig::Redis { url, ttl_seconds: 300, key_prefix: Some("ccr_test".to_string()) };
	...
	fn redis_get_refreshes_idle_ttl() {
		...
		let store = RedisCcrStore::open_with_prefix(&url, "ccr_test_sliding".to_string(), 2).expect("open redis store");
