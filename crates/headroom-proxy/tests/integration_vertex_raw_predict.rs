...
const VERTEX_PATH_REGEX: &str = r"^/v1beta1/projects/[^/]+/locations/[^/]+/publishers/anthropic/models/[^/]+$";
...
async fn native_envelope_round_trip_byte_equal() {
	...
	let got = captured.body.lock().unwrap().clone().expect("upstream got body");
	...
	assert!(parsed.get("model").is_none(), "Vertex envelope must NOT carry a `model` field");
...
async fn thinking_block_preserved() {
	...
	assert!(
		assistant_blocks[0]["signature"].as_str().unwrap().starts_with("EuYBCkQYBCKMAQ"),
...
async fn adc_failure_returns_5xx_no_silent_forward() {
	...
	impl TokenSource for AlwaysFail {
		async fn bearer(&self) -> Result<String, TokenSourceError> {
			Err(TokenSourceError::Fetch("synthetic test failure: ADC chain unreachable".into()))
		}
	}
