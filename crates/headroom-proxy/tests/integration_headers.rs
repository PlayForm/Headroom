...
async fn custom_headers_pass_through_both_ways() {
	...
	Mock::given(method("GET"))
		...
		.respond_with(move |req: &wiremock::Request| {
			...
			let xff = req.headers.get("x-forwarded-for").unwrap().to_str().unwrap();
	...
	assert!(resp.headers().get("connection").is_none(), "hop-by-hop must be stripped");
...
async fn x_headroom_request_headers_stripped() {
	...
	Mock::given(method("POST"))
		...
		.respond_with(move |req: &wiremock::Request| {
			...
			assert!(req.headers.get("x-headroom-mode").is_none(), "x-headroom-mode leaked upstream");
			assert!(
	...
	Mock::given(method("POST"))
		...
		.respond_with(move |req: &wiremock::Request| {
			...
			for hdr in [
				...
				assert!(req.headers.get(hdr).is_none(), "internal header {hdr} leaked upstream");
			}
			ResponseTemplate::new(200).set_body_string("{}")
		})
		.mount(&upstream)
...
async fn xff_appends_existing_value() {
	...
	Mock::given(method("GET"))
		...
		.respond_with(move |req: &wiremock::Request| {
			...
			assert!(xff.starts_with("1.2.3.4"), "expected appended xff, got: {xff}");
			assert!(xff.contains("127.0.0.1"));
			ResponseTemplate::new(200)
		})
		.mount(&upstream)
