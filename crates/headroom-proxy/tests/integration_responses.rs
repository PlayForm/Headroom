...
async fn apply_patch_output_above_2kb_compressed() {
	...
	for i in 0..200 {
		log.push_str(&format!("[2024-01-01 00:00:00] INFO build.rs:42 compiled module foo_{i}\n"));
	}
...
async fn local_shell_output_compressed() {
	...
	let mut log = String::new();
	for i in 0..120 {
		log.push_str(&format!("[2024-01-01 12:00:00] INFO daemon.rs:88 task_{i} completed in 12ms\n"));
	}
	assert!(log.len() > 4096);
