...
async fn tool_message_compressed() {
	...
	assert!(tool_payload.len() > 1024, "must exceed JSON array threshold");
...
async fn streaming_tool_call_argument_accumulation() {
	...
	assert_eq!(tc.id.as_deref(), Some("call_xyz"), "id must persist across chunks (P4-48)");
	assert_eq!(tc.function_name.as_deref(), Some("echo"));
	let parsed: Value = serde_json::from_str(&tc.function_arguments).expect("concatenated arguments must parse");
