	...
	Mock::given(method("POST"))
		...
		.respond_with(move |req: &wiremock::Request| {
			...
			c.authorization = req
				...
				.map(str::to_string);
			c.x_amz_date = req.headers.get("x-amz-date").and_then(|v| v.to_str().ok()).map(str::to_string);
			c.x_amz_content_sha256 = req
				.headers
				.get("x-amz-content-sha256")
				.and_then(|v| v.to_str().ok())
				.map(str::to_string);
			c.host = req.headers.get("host").and_then(|v| v.to_str().ok()).map(str::to_string);
			c.content_type = req
				.headers
				.get("content-type")
				.and_then(|v| v.to_str().ok())
...
async fn redacted_thinking_preserved() {
	...
	assert_byte_equal_sha256(&body, got.body.as_deref().unwrap());
	let parsed: Value = serde_json::from_slice(got.body.as_deref().unwrap()).unwrap();
	assert_eq!(parsed["messages"][1]["content"][0]["type"], "redacted_thinking");
	assert_eq!(
