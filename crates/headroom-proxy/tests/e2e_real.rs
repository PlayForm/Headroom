...
async fn e2e_health_through_full_chain() {
	...
	let r = reqwest::get(format!("{}/healthz", proxy.url())).await.unwrap();
	assert_eq!(r.status(), 200);
	let json: Value = r.json().await.unwrap();
	assert_eq!(json["service"], "headroom-proxy");

	// Rust /healthz/upstream pings Python /healthz.
	let r = reqwest::get(format!("{}/healthz/upstream", proxy.url())).await.unwrap();
	assert_eq!(r.status(), 200);

	// /livez is forwarded to Python.
	let r = reqwest::get(format!("{}/livez", proxy.url())).await.unwrap();
	assert_eq!(r.status(), 200);
...
async fn e2e_openai_non_streaming() {
	...
	assert!(content.to_uppercase().contains("PONG"), "expected PONG, got: {content}");
