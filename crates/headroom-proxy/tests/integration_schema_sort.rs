...
async fn payg_request_with_shuffled_schema_keys_arrives_sorted() {
	...
	let upstream_body = captured.lock().unwrap().clone().expect("upstream should have captured");
	...
	let consts: Vec<&str> = one_of.iter().map(|v| v.get("const").and_then(Value::as_str).unwrap()).collect();
