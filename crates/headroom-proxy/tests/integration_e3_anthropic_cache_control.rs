...
async fn payg_body_without_markers_gets_marker_on_last_tool() {
	...
	let upstream_received = captured.lock().unwrap().clone().expect("upstream captured a body");
	let upstream_parsed: serde_json::Value = serde_json::from_slice(&upstream_received).expect("upstream body is JSON");
	...
	assert_eq!(upstream_parsed.get("model"), payload.get("model"), "model field preserved",);
	assert_eq!(upstream_parsed.get("system"), payload.get("system"), "system field preserved",);
	assert_eq!(
		upstream_parsed.get("messages"),
		payload.get("messages"),
		"messages field preserved",
	);
	assert_eq!(upstream_parsed.pointer("/tools/0/name"), payload.pointer("/tools/0/name"),);
	assert_eq!(upstream_parsed.pointer("/tools/2/name"), payload.pointer("/tools/2/name"),);
...
mod tracing_capture {
	...
	async fn payg_apply_emits_e3_applied_event() {
		...
		assert!(logs.contains("e3_applied"), "expected e3_applied event in logs; got: {logs}",);
		assert!(
			logs.contains(r#""placed_count":1"#),
			"expected placed_count=1 in logs; got: {logs}",
		);
		assert!(logs.contains("tools[0]"), "expected location tools[0] in logs; got: {logs}",);
