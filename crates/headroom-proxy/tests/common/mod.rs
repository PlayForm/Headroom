...
/// hit real GCP.
#[allow(dead_code)]
pub async fn start_proxy_with_state<F, G>(upstream: &str, customize: F, customize_state: G) -> ProxyHandle
where
	F: FnOnce(&mut Config),
	G: FnOnce(AppState) -> AppState,
{
	...
	ProxyHandle { addr, shutdown: Some(tx), task }
}
...
pub fn install_static_token_source(mut state: AppState, bearer: &str) -> AppState {
	state.vertex_token_source =
		Arc::new(headroom_proxy::vertex::StaticTokenSource::new(bearer.to_string())) as Arc<dyn TokenSource>;
	state
}
