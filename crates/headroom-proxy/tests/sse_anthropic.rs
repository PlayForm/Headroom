...
fn run(state: &mut AnthropicStreamState, raw: &[u8]) {
	...
	while let Some(r) = framer.next_event() {
		...
		state.apply(ev).expect("state machine must not fail on valid inputs");
	}
}
...
fn mid_stream_error_event_handled() {
	...
	assert_eq!(s.status, StreamStatus::Errored, "error event must transition status to Errored");
...
fn _ref_event() -> headroom_proxy::sse::SseEvent {
	headroom_proxy::sse::SseEvent { event_name: None, data: Bytes::from_static(b"") }
}
