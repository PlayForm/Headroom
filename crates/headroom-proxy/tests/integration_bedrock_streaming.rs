...
async fn mount_eventstream_upstream(upstream: &MockServer, body: Bytes) {
	Mock::given(method("POST"))
		.and(path(format!("/model/{TEST_MODEL}/invoke-with-response-stream")))
		.respond_with(
...
async fn eventstream_translated_to_sse() {
	...
	let first_frame_pos = body_str.find("\"message_start\"").expect("message_start in stream");
	let last_frame_pos = body_str.find("\"message_stop\"").expect("message_stop in stream");
...
async fn usage_extracted_from_translated_stream() {
	...
	.unwrap();
	let resp = reqwest::Client::new()
		.post(format!("{}/model/{TEST_MODEL}/invoke-with-response-stream", proxy.url()))
		.header("content-type", "application/json")
		.header("accept", "text/event-stream")
		.body(body)
		.send()
...
async fn client_can_choose_eventstream_or_sse() {
	...
	assert!(sse_str.contains("data: "), "sse output must contain data: frames");
...
async fn converse_stream_route_translates_to_sse() {
	...
	let resp = reqwest::Client::new()
		.post(format!("{}/model/{TEST_MODEL}/converse-stream", proxy.url()))
		.header("content-type", "application/json")
		.header("accept", "text/event-stream")
		.body(body)
		.send()
