...
fn count_lines_with_labels(scrape: &str, metric: &str, label_pairs: &[(&str, &str)]) -> Option<u64> {
...
async fn metrics_increment_per_invoke() {
	...
	for _ in 0..3 {
		let resp = reqwest::Client::new()
			.post(format!("{}/model/{TEST_MODEL_INVOKE_COUNT}/invoke", proxy.url()))
...
async fn metrics_observe_latency() {
	...
	let after_count = count_lines_with_labels(
		...
		&[("model", TEST_MODEL_LATENCY), ("region", TEST_REGION_LATENCY)],
	)
...
async fn eventstream_metrics_per_message_type() {
	...
	Mock::given(method("POST"))
		.and(path(format!("/model/{TEST_MODEL_EVENTSTREAM}/invoke-with-response-stream")))
		.respond_with(
...
async fn metrics_endpoint_serves_scrape() {
	...
	Mock::given(method("POST"))
		.and(path(format!("/model/{TEST_MODEL_SCRAPE}/invoke-with-response-stream")))
		.respond_with(
			ResponseTemplate::new(200)
				.insert_header("content-type", "application/vnd.amazon.eventstream")
				.set_body_bytes(chunks.to_vec()),
	...
	let stream_resp = reqwest::Client::new()
		.post(format!("{}/model/{TEST_MODEL_SCRAPE}/invoke-with-response-stream", proxy.url()))
		.header("content-type", "application/json")
		.body(body)
		.send()
		.await
