...
async fn start_simulator_with_config(config: SimulatorConfig) -> SimulatorHandle {
	...
	SimulatorHandle { addr, shutdown: Some(tx), task }
}
...
async fn start_simulator_proxy(simulator_url: &str) -> common::ProxyHandle {
	...
	start_proxy_with_state(
		...
		|s| install_static_token_source(s.with_bedrock_credentials(test_credentials()), TEST_BEARER),
	)
	.await
}

async fn start_simulator_proxy_without_bedrock_credentials(simulator_url: &str) -> common::ProxyHandle {
	let bedrock_endpoint: Url = simulator_url.parse().expect("simulator url");
	start_proxy_with_state(
		simulator_url,
		|c| {
...
fn assert_simulator_header(resp: &reqwest::Response) {
	assert_eq!(
		resp.headers().get("x-headroom-simulator").and_then(|v| v.to_str().ok()),
		Some("true"),
...
fn json_error_stub(name: &str, path: &str, body_contains: &str, status: u16, error_type: &str) -> StubRule {
	StubRule {
		name: name.to_string(),
...
async fn json_post(client: &reqwest::Client, url: impl Into<String>, body: Value) -> reqwest::Response {
	client
		.post(url.into())
...
async fn anthropic_messages_json_and_stream_use_simulator() {
	...
	assert_eq!(body["content"][0]["text"], json!("simulated anthropic response"));
...
async fn openai_chat_responses_and_conversations_use_simulator() {
	...
	let listed: Value = client
		.get(format!("{}/v1/conversations/conv_sim_0001/items", proxy.url()))
		.send()
		.await
		.unwrap()
		.json()
	...
	let delete = client
		.delete(format!("{}/v1/conversations/conv_sim_0001/items/item_sim_0001", proxy.url()))
		.send()
		.await
		.unwrap();
...
async fn bedrock_invoke_converse_and_streaming_use_simulator() {
	...
	for action in ["invoke", "converse"] {
		let resp = json_post(&client, format!("{}/model/{BEDROCK_MODEL}/{action}", proxy.url()), body.clone()).await;
		assert_eq!(resp.status(), 200, "{action}");
		assert_simulator_header(&resp);
		let json: Value = resp.json().await.unwrap();
		assert_eq!(json["role"], json!("assistant"));
		assert_eq!(json["content"][0]["text"], json!("simulated bedrock anthropic response"));
	}

	let sse = json_post(
		&client,
		format!("{}/model/{BEDROCK_MODEL}/invoke-with-response-stream", proxy.url()),
		body.clone(),
	)
	.await;
	...
	let binary = client
		.post(format!("{}/model/{BEDROCK_MODEL}/converse-stream", proxy.url()))
		.header("content-type", "application/json")
...
async fn vertex_raw_and_stream_predict_use_simulator() {
	...
	assert_eq!(raw_body["model"], json!("headroom-simulator-vertex-anthropic"));
...
async fn headroom_preflight_errors_stop_before_simulator_fallback() {
	...
	assert_eq!(bedrock_body["error"]["type"], json!("bedrock_credentials_missing"));
