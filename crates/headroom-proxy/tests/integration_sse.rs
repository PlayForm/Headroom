							...
							async move {
								...
								tokio::spawn(async move {
									for i in 0..10u32 {
										...
										if tx.send(Ok(Frame::data(Bytes::from(payload)))).await.is_err() {
...
async fn sse_chunks_arrive_with_preserved_timing() {
	...
	let resp = reqwest::Client::new().get(format!("{}/sse", proxy.url())).send().await.unwrap();
...
async fn client_disconnect_propagates_to_upstream() {
	...
	let resp = client.get(format!("{}/sse", proxy.url())).send().await.unwrap();
	let mut stream = resp.bytes_stream();
