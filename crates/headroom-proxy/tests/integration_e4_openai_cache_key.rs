...
fn captured_body(captured: &Arc<Mutex<Option<Vec<u8>>>>) -> Vec<u8> {
	captured.lock().unwrap().clone().expect("upstream should have captured a body")
}
...
async fn payg_chat_completions_injects_prompt_cache_key() {
	...
	assert!(key.bytes().all(|b| b.is_ascii_hexdigit()), "key must be hex: {key}");

	// Other fields preserved structurally (parse-equivalent).
	assert_eq!(received_json.get("model").and_then(Value::as_str), Some("gpt-4o"));
...
async fn payg_chat_with_customer_set_key_preserves_value() {
	...
	let received_json = parse_json(&received);
	assert_eq!(
		received_json.get("prompt_cache_key").and_then(Value::as_str),
