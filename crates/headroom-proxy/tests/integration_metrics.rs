...
fn find_value_with_labels(scrape: &str, metric: &str, label_pairs: &[(&str, &str)]) -> Option<f64> {
	for line in scrape.lines() {
		...
		if !label_pairs.iter().all(|(k, v)| line.contains(&format!("{k}=\"{v}\""))) {
			continue;
		}
...
async fn cache_hit_rate_emitted_per_session() {
	...
	// load-bearing one.
	let count = find_value_with_labels(&scrape, "proxy_cache_hit_rate_per_session_count", &[("provider", "anthropic")])
		.expect("histogram count must appear after first session");
	assert!(count >= 1.0, "expected ≥1 observation on anthropic label; got {count}");
	let sum = find_value_with_labels(&scrape, "proxy_cache_hit_rate_per_session_sum", &[("provider", "anthropic")])
...
async fn compression_ratio_emitted_per_strategy() {
	...
	let ratio_count = find_value_with_labels(
		...
		&[("strategy", TEST_STRATEGY), ("content_type", TEST_CONTENT_TYPE)],
	)
...
async fn passthrough_bytes_modified_zero_when_no_compression() {
	...
	let init_row = find_value_with_labels(&scrape, "proxy_passthrough_bytes_modified_total", &[("path", "__init__")])
	...
	let messages_row =
		find_value_with_labels(&scrape, "proxy_passthrough_bytes_modified_total", &[("path", "/v1/messages")]);
	assert!(
	...
	assert!(touched >= 7.0, "expected ≥7 byte delta after touch; got {touched}");
...
async fn passthrough_bytes_modified_alarm_fires_with_byte_delta_label() {
	...
	let proxy = start_proxy_with("http://127.0.0.1:1", |_| {}).await;
	let scrape = scrape_metrics(&proxy.url()).await;
	let value = find_value_with_labels(&scrape, "proxy_passthrough_bytes_modified_total", &[("path", TEST_PATH)])
...
async fn service_tier_logged_known_value() {
	...
	let scrape = scrape_metrics(&proxy.url()).await;
	let tier_count = find_value_with_labels(&scrape, "proxy_service_tier_count_total", &[("tier", "priority")])
		.expect("service tier counter row must appear under 'priority'");
	assert!(tier_count >= 1.0, "expected ≥1 service_tier increment; got {tier_count}");
...
async fn service_tier_unknown_bucketed_to_other() {
	...
	let other_count = find_value_with_labels(&scrape, "proxy_service_tier_count_total", &[("tier", "other")])
...
fn service_tier_validate_unknown_returns_other_sentinel() {
	...
	assert_eq!(service_tier::validate("nonsense_value"), service_tier::OTHER);
...
async fn incomplete_status_logged_with_reason() {
	...
	let proxy = start_proxy_with("http://127.0.0.1:1", |_| {}).await;
	let scrape = scrape_metrics(&proxy.url()).await;
	let count = find_value_with_labels(&scrape, "proxy_response_status_count_total", &[("status", "incomplete")])
...
async fn rate_limit_snapshot_emits_gauges() {
	...
	let v = find_value_with_labels(&scrape, "proxy_rate_limit_remaining_requests", &[("provider", "anthropic")])
		.expect("requests gauge row");
	assert!(v >= 1.0);
	let v = find_value_with_labels(&scrape, "proxy_rate_limit_remaining_tokens", &[("provider", "anthropic")])
