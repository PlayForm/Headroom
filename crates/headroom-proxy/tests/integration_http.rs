...
async fn all_methods_round_trip_with_body() {
	...
	let head_resp = client.head(format!("{}/echo", proxy.url())).send().await.unwrap();
...
async fn upstream_error_codes_passthrough() {
	...
	let client = reqwest::Client::new();
	for status in [404u16, 500, 502] {
		let resp = client.get(format!("{}/code/{status}", proxy.url())).send().await.unwrap();
...
async fn query_string_preserved() {
	...
	let proxy = start_proxy(&upstream.uri()).await;
	let resp = reqwest::get(format!("{}/q?a=1&b=two", proxy.url())).await.unwrap();
...
async fn one_mb_post_streams_through() {
	...
	Mock::given(method("POST"))
		...
		.respond_with(move |req: &wiremock::Request| {
			assert_eq!(req.body.len(), payload_clone.len(), "upstream got full body");
