...
mod tracing_capture {
	...
	async fn volatile_timestamp_in_system_emits_warn_and_passes_through() {
		...
		let upstream_received = captured.lock().unwrap().clone().expect("upstream should have captured a body");
		assert_eq!(upstream_received, body, "volatile detector must not mutate the request body",);
