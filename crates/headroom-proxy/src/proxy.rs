...
pub(crate) async fn forward_http(
	...
	// both branches clean.
	let headers_snapshot = if should_intercept { Some(req.headers().clone()) } else { None };
	...
	let upstream_resp = if should_intercept {
		...
		let endpoint = compression::classify_compressible_path(uri.path()).expect("is_compressible_path guarded above");
		...
		if let Ok(parsed) = serde_json::from_slice::<serde_json::Value>(&buffered) {
			...
			let findings = cache_stabilization::volatile_detector::detect_volatile_content(&parsed, volatile_kind);
			if !findings.is_empty() {
				cache_stabilization::volatile_detector::emit_volatile_warnings(&findings, &request_id);
			}
		...
		let outcome = match endpoint {
			...
			compression::CompressibleEndpoint::OpenAiResponses => compression::compress_openai_responses_request(
				&buffered,
				state.config.compression_mode,
				auth_mode,
				&request_id,
		...
		let body_to_send = match endpoint {
			...
			| compression::CompressibleEndpoint::OpenAiResponses => {
				...
				maybe_inject_openai_prompt_cache_key(body_to_send, shape, auth_mode, &request_id, &path_for_log)
			},
		...
		let body_stream = TryStreamExt::map_err(req.into_body().into_data_stream(), std::io::Error::other);
	...
	let sse_kind = if is_sse {
		...
		if matches!(kind, SseStreamKind::OpenAiResponses) && !state.config.enable_responses_streaming {
			tracing::info!(
				request_id = %request_id,
				path = %path_for_log,
	...
	let rate_limit_snapshot = crate::observability::extract_rate_limit_snapshot(upstream_resp.headers());
	...
	if let Some(provider) = rate_limit_provider {
		crate::observability::record_rate_limit_snapshot(provider, &rate_limit_snapshot, &request_id);
	...
	let response = response.body(body).map_err(|e| ProxyError::InvalidHeader(e.to_string()))?;
...
fn maybe_inject_openai_prompt_cache_key(
	...
	use cache_stabilization::openai_cache_key::{inject_prompt_cache_key, InjectOutcome, SkipReason};
	...
	match kind {
		...
		SseStreamKind::OpenAiResponses => {
			...
			if let Some(status) = state.terminal_status() {
				crate::observability::record_response_status(status, state.incomplete_reason.as_deref(), &request_id);
			}
			tracing::info!(
				request_id = %request_id,
				provider = "openai_responses",
...
pub async fn body_to_bytes(body: Body) -> Result<Bytes, axum::Error> {
	use axum::Error;
	body.collect().await.map(|c| c.to_bytes()).map_err(Error::new)
}
