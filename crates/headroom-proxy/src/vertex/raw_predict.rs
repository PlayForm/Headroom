...
pub(crate) async fn forward_vertex_request(
	...
	let buffered = match to_bytes(body, max).await {
		...
		Err(e) => {
			...
			return error_response(StatusCode::PAYLOAD_TOO_LARGE, "request body exceeds buffer limit");
		},
	};
...
/// on this loop.
async fn run_anthropic_sse_state_machine(mut rx: tokio::sync::mpsc::Receiver<bytes::Bytes>, request_id: String) {
