...
pub fn classify_items<'a>(items_raw: &'a RawValue) -> Result<Vec<ClassifiedItem<'a>>, ClassifyError> {
	...
	for raw in raw_items {
		let type_only: TypeOnly<'a> = serde_json::from_str(raw.get()).map_err(|_| ClassifyError::ItemMissingTypeTag)?;
		...
		out.push(ClassifiedItem { typed, type_tag, raw });
	}
	Ok(out)
}
