...
use super::metric_names::{
	LABEL_PATH, LABEL_PROVIDER, LABEL_STATUS, LABEL_TIER, METRIC_PROXY_PASSTHROUGH_BYTES_MODIFIED_TOTAL,
	METRIC_PROXY_PASSTHROUGH_BYTES_MODIFIED_TOTAL_HELP, METRIC_PROXY_RATE_LIMIT_REMAINING_INPUT_TOKENS,
	METRIC_PROXY_RATE_LIMIT_REMAINING_INPUT_TOKENS_HELP, METRIC_PROXY_RATE_LIMIT_REMAINING_OUTPUT_TOKENS,
	METRIC_PROXY_RATE_LIMIT_REMAINING_OUTPUT_TOKENS_HELP, METRIC_PROXY_RATE_LIMIT_REMAINING_REQUESTS,
	METRIC_PROXY_RATE_LIMIT_REMAINING_REQUESTS_HELP, METRIC_PROXY_RATE_LIMIT_REMAINING_TOKENS,
	METRIC_PROXY_RATE_LIMIT_REMAINING_TOKENS_HELP, METRIC_PROXY_RESPONSE_STATUS_COUNT_TOTAL,
...
pub fn record_rate_limit_snapshot(provider: &'static str, snapshot: &RateLimitSnapshot, request_id: &str) {
...
pub fn service_tier_counter(registry: &Registry) -> &'static IntCounterVec {
	...
	COUNTER.get_or_init(|| {
		...
		let counter =
			IntCounterVec::new(opts, &[LABEL_TIER]).expect("proxy_service_tier_count_total descriptor is well-formed");
		registry
			.register(Box::new(counter.clone()))
...
mod tests {
	...
	fn extract_rate_limit_snapshot_anthropic() {
		...
		h.insert("anthropic-ratelimit-requests-remaining", HeaderValue::from_static("499"));
		h.insert("anthropic-ratelimit-tokens-remaining", HeaderValue::from_static("99000"));
		h.insert("anthropic-ratelimit-input-tokens-remaining", HeaderValue::from_static("80000"));
		h.insert("anthropic-ratelimit-output-tokens-remaining", HeaderValue::from_static("16000"));
	...
	fn extract_rate_limit_snapshot_openai() {
		let mut h = HeaderMap::new();
		h.insert("x-ratelimit-remaining-requests", HeaderValue::from_static("1000"));
		h.insert("x-ratelimit-remaining-tokens", HeaderValue::from_static("250000"));
		let snap = extract_rate_limit_snapshot(&h);
