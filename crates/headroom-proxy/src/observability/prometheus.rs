...
pub fn record_bedrock_invoke(model: &str, region: &str, auth_mode: AuthMode) {
	invoke_counter().with_label_values(&[model, region, auth_mode.as_str()]).inc();
	tracing::debug!(
...
pub fn observe_bedrock_invoke_latency(model: &str, region: &str, seconds: f64) {
	invoke_latency().with_label_values(&[model, region]).observe(seconds);
	tracing::debug!(
		event = "metric_recorded",
...
pub fn record_bedrock_eventstream_message(model: &str, region: &str, event_type: &str) {
	eventstream_counter().with_label_values(&[model, region, event_type]).inc();
	tracing::debug!(
		event = "metric_recorded",
...
pub async fn handle_metrics() -> Response {
	...
	rejected_counter.with_label_values(&[INIT_SENTINEL]).inc_by(0);
	passthrough_counter.with_label_values(&[INIT_SENTINEL]).inc_by(0);
...
mod tests {
	...
	fn invoke_counter_advertises_metric_in_scrape() {
		...
		invoke_counter()
			.with_label_values(&["anthropic.unit-test-advertise-v1:0", "us-test-advertise-1", "oauth"])
			.inc();
		let body = scrape();
		assert!(
	...
	fn invoke_increment_appears_with_labels() {
		record_bedrock_invoke("anthropic.claude-3-haiku-20240307-v1:0", "us-east-1", AuthMode::OAuth);
		let body = scrape();
		...
		assert!(body.contains("auth_mode=\"oauth\""), "scrape missing auth_mode label: {body}");
		assert!(
			body.contains("model=\"anthropic.claude-3-haiku-20240307-v1:0\""),
			"scrape missing model label: {body}"
		);
		assert!(body.contains("region=\"us-east-1\""), "scrape missing region label: {body}");
	}
	...
	fn eventstream_counter_records_event_type_label() {
		record_bedrock_eventstream_message("anthropic.claude-3-haiku-20240307-v1:0", "us-east-1", "chunk");
		record_bedrock_eventstream_message("anthropic.claude-3-haiku-20240307-v1:0", "us-east-1", "metadata");
		let body = scrape();
		assert!(body.contains("event_type=\"chunk\""), "scrape missing event_type=chunk: {body}");
		assert!(
