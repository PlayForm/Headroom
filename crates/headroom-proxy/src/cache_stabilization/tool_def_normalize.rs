...
mod tests {
	...
	fn sort_alphabetic_by_name() {
		let mut tools = vec![json!({"name": "B"}), json!({"name": "A"}), json!({"name": "C"})];
	...
	fn idempotent_resort_no_change() {
		let mut tools = vec![json!({"name": "A"}), json!({"name": "B"}), json!({"name": "C"})];
		let changed = sort_tools_deterministically(&mut tools);
	...
	fn sort_alphabetic_by_openai_function_name() {
		...
		let names: Vec<&str> = tools
			.iter()
			.map(|t| t.get("function").and_then(|f| f.get("name")).and_then(Value::as_str).unwrap())
			.collect();
	...
	fn unnamed_tool_uses_md5_fallback() {
		...
		// across runs.
		let mut tools = vec![json!({"description": "second"}), json!({"description": "first"})];
		let _ = sort_tools_deterministically(&mut tools);
		let bytes_run1 = serde_json::to_vec(&tools).unwrap();

		let mut tools2 = vec![json!({"description": "first"}), json!({"description": "second"})];
	...
	fn cache_control_detection_finds_marker() {
		...
		let without_marker = vec![json!({"name": "A"}), json!({"name": "B"}), json!({"name": "C"})];
	...
	fn preserves_array_order_in_oneof() {
		...
		let consts: Vec<&str> = arr.iter().map(|v| v.get("const").and_then(Value::as_str).unwrap()).collect();
		assert_eq!(
			consts,
	...
	fn does_not_alter_arrays_within_arrays() {
		...
		let inner_nums: Vec<i64> = outer[0].as_array().unwrap().iter().map(|v| v.as_i64().unwrap()).collect();
		assert_eq!(inner_nums, vec![3, 1, 2]);
		let inner_strs: Vec<&str> = outer[1].as_array().unwrap().iter().map(|v| v.as_str().unwrap()).collect();
