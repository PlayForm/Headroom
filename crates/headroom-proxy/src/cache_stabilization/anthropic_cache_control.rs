...
pub fn auto_place_anthropic_cache_control(body: &mut Value) -> AutoPlaceOutcome {
	...
	if any_anthropic_cache_control(body) {
		return AutoPlaceOutcome::Skipped { reason: SkipReason::MarkerPresent };
	}
	...
	let tools = match body.get_mut("tools") {
		...
		_ => {
			...
			return AutoPlaceOutcome::Applied { placed_count: 0, locations: Vec::new() };
		},
	};
	...
	AutoPlaceOutcome::Applied { placed_count: 1, locations: vec![format!("tools[{last_idx}]")] }
}
...
mod tests {
	...
	fn places_cache_control_on_last_tool_when_payg_and_no_markers() {
		...
		match outcome {
			AutoPlaceOutcome::Applied { placed_count, locations } => {
		...
		let cc = body.pointer("/tools/0/cache_control").expect("marker inserted on tools[0]");
	...
	fn skips_when_any_tool_already_has_marker() {
		...
		let outcome = auto_place_anthropic_cache_control(&mut body);
		assert_eq!(outcome, AutoPlaceOutcome::Skipped { reason: SkipReason::MarkerPresent });
	...
	fn idempotent_when_we_already_placed_marker_last_run() {
		...
		assert!(matches!(first, AutoPlaceOutcome::Applied { placed_count: 1, .. }));
		let after_first = body.clone();
		let second = auto_place_anthropic_cache_control(&mut body);
		assert_eq!(second, AutoPlaceOutcome::Skipped { reason: SkipReason::MarkerPresent });
	...
	fn does_nothing_when_no_tools_present() {
		...
		let before = body.clone();
		let outcome = auto_place_anthropic_cache_control(&mut body);
		assert_eq!(outcome, AutoPlaceOutcome::Applied { placed_count: 0, locations: Vec::new() });
	...
	fn system_string_form_does_not_get_converted_to_array() {
		...
		let outcome = auto_place_anthropic_cache_control(&mut body);
		assert!(matches!(outcome, AutoPlaceOutcome::Applied { placed_count: 1, .. }));
		...
		assert_eq!(body.pointer("/tools/0/cache_control"), Some(&json!({"type": "ephemeral"})),);
	}
	...
	fn applies_only_one_marker_in_first_ship_default() {
		...
		assert_eq!(
			outcome,
			AutoPlaceOutcome::Applied { placed_count: 1, locations: vec!["tools[1]".to_string()] }
		);
		...
		for (i, msg) in body.pointer("/messages").unwrap().as_array().unwrap().iter().enumerate() {
		...
		assert!(body.pointer("/tools/0/cache_control").is_none());
		assert_eq!(body.pointer("/tools/1/cache_control"), Some(&json!({"type": "ephemeral"})),);
	}
	...
	fn applied_path_preserves_other_tool_fields() {
		...
		let tools = body.get_mut("tools").and_then(Value::as_array_mut).expect("tools array");
		for tool in tools {
