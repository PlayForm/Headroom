...
pub fn inject_prompt_cache_key(body: &mut Value, shape: OpenAiShape) -> InjectOutcome {
	...
	if !body.is_object() {
		return InjectOutcome::Skipped { reason: SkipReason::NotAnObject };
	}

	if has_prompt_cache_key(body) {
		return InjectOutcome::Skipped { reason: SkipReason::KeyPresent };
	}
...
fn first_system_message_content(body: &Value, key: &str) -> Value {
	body.get(key).and_then(first_system_in_array).unwrap_or(Value::Null)
}
...
mod tests {
	...
	fn skips_when_key_already_set() {
		...
		let outcome = inject_prompt_cache_key(&mut body, OpenAiShape::ChatCompletions);
		assert_eq!(outcome, InjectOutcome::Skipped { reason: SkipReason::KeyPresent });
	...
	fn skips_when_key_already_set_responses() {
		...
		let mut body = original.clone();
		let outcome = inject_prompt_cache_key(&mut body, OpenAiShape::Responses);
		assert!(matches!(outcome, InjectOutcome::Skipped { reason: SkipReason::KeyPresent }));
	...
	fn skips_when_body_is_not_an_object() {
		...
		let outcome = inject_prompt_cache_key(&mut body, OpenAiShape::ChatCompletions);
		assert_eq!(outcome, InjectOutcome::Skipped { reason: SkipReason::NotAnObject });
	}
	...
	fn key_length_is_exactly_32_hex_chars() {
		...
		for (i, base) in inputs.iter().enumerate() {
			...
			let outcome = inject_prompt_cache_key(&mut body, OpenAiShape::ChatCompletions);
			assert!(matches!(outcome, InjectOutcome::Applied { .. }), "input {i}: expected Applied");
			let key = injected_key(&body);
			assert_eq!(key.len(), KEY_HEX_LEN, "input {i}: key {key:?} must be {KEY_HEX_LEN} chars");
	...
	fn has_prompt_cache_key_works() {
		...
		assert!(has_prompt_cache_key(&json!({"prompt_cache_key": "user-pinned"})));
	}
