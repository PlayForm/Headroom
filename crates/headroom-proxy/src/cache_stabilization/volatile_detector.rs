...
impl ApiKind {
	...
	pub fn from_endpoint(endpoint: CompressibleEndpoint) -> Self {
		match endpoint {
			...
			CompressibleEndpoint::OpenAiChatCompletions | CompressibleEndpoint::OpenAiResponses => ApiKind::OpenAi,
		}
	}
}
...
fn looks_like_iso8601(window: &[u8]) -> bool {
	...
	let digits_in = |range: std::ops::Range<usize>| -> bool { window[range].iter().all(u8::is_ascii_digit) };
	digits_in(0..4)
		&& window[4] == b'-'
		&& digits_in(5..7)
		&& window[7] == b'-'
...
mod tests {
	...
	fn from_endpoint_maps_correctly() {
		...
		assert_eq!(ApiKind::from_endpoint(CompressibleEndpoint::OpenAiResponses), ApiKind::OpenAi,);
	}
}
