...
pub fn strip_internal_headers(headers: &mut HeaderMap) -> usize {
	let to_remove: Vec<HeaderName> = headers.keys().filter(|n| is_internal_header(n)).cloned().collect();
...
pub fn build_forward_request_headers(
	...
	set_single(&mut out, HeaderName::from_static("x-forwarded-proto"), forwarded_proto);
	if let Some(host) = forwarded_host {
		set_single(&mut out, HeaderName::from_static("x-forwarded-host"), host);
	}
	set_single(&mut out, HeaderName::from_static("x-request-id"), request_id);
	out
}
...
mod tests {
	...
	fn internal_header_detected_case_insensitive() {
		...
		assert!(is_internal_header(&HeaderName::from_bytes(b"X-Headroom-Mode").unwrap()));
		assert!(is_internal_header(&HeaderName::from_bytes(b"X-HEADROOM-FOO").unwrap()));
		assert!(!is_internal_header(&HeaderName::from_static("x-request-id")));
		assert!(!is_internal_header(&HeaderName::from_static("authorization")));
	}
	...
	fn build_forward_strips_internal_when_enabled() {
		...
		let out =
			build_forward_request_headers(&incoming, "127.0.0.1".parse().unwrap(), "http", Some("h"), "req-1", true);
	...
	fn build_forward_keeps_internal_when_disabled() {
		...
		incoming.insert("x-headroom-bypass", HeaderValue::from_static("true"));
		let out =
			build_forward_request_headers(&incoming, "127.0.0.1".parse().unwrap(), "http", Some("h"), "req-1", false);
		assert!(out.get("authorization").is_some());
