...
pub fn compress_anthropic_request(
	...
	if matches!(auth_mode, RequestAuthMode::Payg) {
		match auto_place_anthropic_cache_control(&mut parsed) {
			...
			AutoPlaceOutcome::Skipped { reason: SkipReason::AuthMode } => {
	...
	match compress_anthropic_live_zone(&dispatch_body, frozen_count, auth_mode.into(), model) {
		Ok(LiveZoneOutcome::NoChange { manifest }) => {
			...
			let blocks_excluded = manifest
				.block_outcomes
				.iter()
				.filter(|b| matches!(b.action, BlockAction::Excluded { reason: ExclusionReason::HotZoneBlockType }))
				.count();
			tracing::info!(
				request_id = %request_id,
				path = "/v1/messages",
...
mod tests {
	...
	fn mode_off_short_circuits_without_parsing() {
		...
		match out {
			Outcome::Passthrough { reason: PassthroughReason::ModeOff } => {},
	...
	fn pr_e3_payg_with_tools_and_no_markers_returns_compressed_with_marker() {
		...
		match out {
			Outcome::Compressed { body: new_body, strategies_applied, markers_inserted, .. } => {
				assert!(
				...
				let parsed: serde_json::Value = serde_json::from_slice(&new_body).expect("re-parse new body");
				assert_eq!(
	...
	fn e1_sorts_tools_when_payg_and_no_marker() {
		...
		match out {
			Outcome::Compressed { body: new_body, strategies_applied, .. } => {
				assert!(
	...
	fn e2_runs_even_when_marker_blocks_e1() {
		...
		match out {
			Outcome::Compressed { strategies_applied, .. } => {
