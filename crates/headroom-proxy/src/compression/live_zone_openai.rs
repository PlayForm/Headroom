...
use headroom_core::transforms::{compress_openai_chat_live_zone, BlockAction, LiveZoneError, LiveZoneOutcome};
...
mod tests {
	...
	fn mode_off_short_circuits() {
		...
		let out = compress_openai_chat_request(&body, CompressionMode::Off, RequestAuthMode::Payg, "req-1");
	...
	fn invalid_json_passthrough() {
		...
		let out = compress_openai_chat_request(&body, CompressionMode::LiveZone, RequestAuthMode::Payg, "req-2");
	...
	fn no_messages_passthrough() {
		...
		let out = compress_openai_chat_request(&body, CompressionMode::LiveZone, RequestAuthMode::Payg, "req-3");
	...
	fn small_body_no_change() {
		...
		let out = compress_openai_chat_request(&body, CompressionMode::LiveZone, RequestAuthMode::Payg, "req-4");
	...
	fn e1_sorts_tools_when_payg() {
		...
		let out = compress_openai_chat_request(&body, CompressionMode::LiveZone, RequestAuthMode::Payg, "req-e1");
		match out {
	...
	fn e1_passes_through_when_oauth() {
		...
		let out =
			compress_openai_chat_request(&body, CompressionMode::LiveZone, RequestAuthMode::OAuth, "req-e1-oauth");
		assert!(matches!(out, Outcome::NoCompression));
	}
