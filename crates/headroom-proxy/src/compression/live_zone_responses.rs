...
use headroom_core::transforms::{
	compress_openai_responses_live_zone, summarize_openai_responses_no_change_reason, BlockAction, LiveZoneError,
	LiveZoneOutcome,
};
...
pub fn compress_openai_responses_request(
	...
	if matches!(mode, CompressionMode::Off) {
		...
		return Outcome::Passthrough { reason: PassthroughReason::ModeOff };
	}
	...
	let parsed: serde_json::Value = match serde_json::from_slice(body) {
		...
		Err(_) => {
			...
			return Outcome::Passthrough { reason: PassthroughReason::NotJson };
		},
	};
	...
	if !has_array_field {
		...
		return Outcome::Passthrough { reason: PassthroughReason::NoMessages };
	}
	...
	let model = parsed.get("model").and_then(serde_json::Value::as_str).unwrap_or(DEFAULT_MODEL);
	...
	match compress_openai_responses_live_zone(&dispatch_body, auth_mode.into(), model) {
			...
			for entry in &manifest.block_outcomes {
				match entry.action {
					BlockAction::Compressed {
						...
						if let Some(slot) = per_strategy_tokens.iter_mut().find(|s| s.strategy == strategy) {
					...
					BlockAction::CompressorError { strategy, ref error } => {
		...
		Err(LiveZoneError::BodyNotJson(_)) => {
			...
			Outcome::Passthrough { reason: PassthroughReason::NotJson }
		},
		Err(LiveZoneError::NoMessagesArray) => {
			...
			Outcome::Passthrough { reason: PassthroughReason::NoMessages }
		},
	}
}
...
fn normalize_tool_definitions_responses(
	...
	for tool in tools.iter_mut() {
		let Some(parameters) = tool.get_mut("function").and_then(|f| f.get_mut("parameters")) else {
			continue;
		};
...
mod tests {
	...
	fn mode_off_short_circuits() {
		...
		let out = compress_openai_responses_request(&body, CompressionMode::Off, RequestAuthMode::Payg, "req-1");
		assert!(matches!(out, Outcome::Passthrough { reason: PassthroughReason::ModeOff }));
	}

	#[test]
	fn invalid_json_passthrough() {
		let body = Bytes::from_static(b"\x01\x02 not json");
		let out = compress_openai_responses_request(&body, CompressionMode::LiveZone, RequestAuthMode::Payg, "req-2");
		assert!(matches!(out, Outcome::Passthrough { reason: PassthroughReason::NotJson }));
	}

	#[test]
	fn no_input_passthrough() {
		let body = body_of(json!({"model": "gpt-4o"}));
		let out = compress_openai_responses_request(&body, CompressionMode::LiveZone, RequestAuthMode::Payg, "req-3");
		assert!(matches!(out, Outcome::Passthrough { reason: PassthroughReason::NoMessages }));
	}
	...
	fn small_body_no_change() {
		...
		let out = compress_openai_responses_request(&body, CompressionMode::LiveZone, RequestAuthMode::Payg, "req-4");
	...
	fn e1_sorts_tools_when_payg() {
		...
		let out =
			compress_openai_responses_request(&body, CompressionMode::LiveZone, RequestAuthMode::Payg, "req-e1-resp");
		match out {
			Outcome::Compressed { strategies_applied, .. } => assert!(
	...
	fn e1_passes_through_when_oauth() {
		...
		let out =
			compress_openai_responses_request(&body, CompressionMode::LiveZone, RequestAuthMode::OAuth, "req-e1-oauth");
		assert!(matches!(out, Outcome::NoCompression));
	}
}
