...
impl AnthropicStreamState {
	...
	fn on_message_start(&mut self, event: &SseEvent) -> Result<(), StateError> {
		...
		let msg = v.get("message").ok_or(StateError::MissingField { field: "message" })?;
	...
	fn on_content_block_start(&mut self, event: &SseEvent) -> Result<(), StateError> {
		...
		let cb = v
			.get("content_block")
			.ok_or(StateError::MissingField { field: "content_block" })?;
		let block_type = cb.get("type").and_then(|x| x.as_str()).unwrap_or("").to_string();
		let mut block = BlockState { block_type, metadata: cb.clone(), ..Default::default() };
	...
	fn on_content_block_delta(&mut self, event: &SseEvent) -> Result<(), StateError> {
		...
		let index = v
			...
			.and_then(|x| x.as_u64())
			.ok_or(StateError::MissingField { field: "index" })? as usize;
		let delta = v.get("delta").ok_or(StateError::MissingField { field: "delta" })?;
		let delta_type = delta
			.get("type")
			.and_then(|x| x.as_str())
			.ok_or(StateError::MissingField { field: "delta.type" })?;
