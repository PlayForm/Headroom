...
impl ResponseState {
	...
	pub fn apply(&mut self, event: SseEvent) -> Result<(), StateError> {
		...
		match name {
			...
			"response.reasoning_summary.done" | "response.reasoning_summary_text.done" | "reasoning_summary.done" => {
				self.on_reasoning_summary_done(&v)
			},
	...
	fn on_response_created(&mut self, v: &Value) -> Result<(), StateError> {
		let resp = v.get("response").ok_or(StateError::MissingField { field: "response" })?;
	...
	fn on_output_item_added(&mut self, v: &Value) -> Result<(), StateError> {
		let item = v.get("item").ok_or(StateError::MissingField { field: "item" })?;
		let id = item
			.get("id")
			.and_then(|x| x.as_str())
			.ok_or(StateError::MissingField { field: "item.id" })?
			.to_string();
		let item_type = item.get("type").and_then(|x| x.as_str()).unwrap_or("").to_string();
		self.items
			.insert(id, ItemState { item_type, metadata: item.clone(), ..Default::default() });
		Ok(())
	}
	...
	fn on_content_part_added(&mut self, v: &Value) -> Result<(), StateError> {
		...
		let part_index = v
			...
			.and_then(|x| x.as_u64())
			.ok_or(StateError::MissingField { field: "content_index" })? as usize;
		let part = v.get("part").ok_or(StateError::MissingField { field: "part" })?;
		let part_type = part.get("type").and_then(|x| x.as_str()).unwrap_or("").to_string();
		let item = self.items.entry(item_id).or_default();
		item.content_parts
			.insert(part_index, ContentPartState { part_type, ..Default::default() });
		Ok(())
	}
	...
	fn on_output_text_delta(&mut self, v: &Value) -> Result<(), StateError> {
		...
		if let Some(part_index) = v.get("content_index").or_else(|| v.get("part_index")).and_then(|x| x.as_u64()) {
