...
pub fn sign_request(inputs: &SigningInputs<'_>) -> Result<SignedHeaders, SigV4Error> {
	...
	let signing_output = sign(signable, &signing_params).map_err(|e| SigV4Error::Sign(e.to_string()))?;
...
mod tests {
	...
	fn signs_minimal_request_produces_expected_headers() {
		...
		let names: Vec<String> = signed.entries.iter().map(|(k, _)| k.to_ascii_lowercase()).collect();
		assert!(
			names.iter().any(|n| n == "authorization"),
			"must add authorization; got {names:?}"
		);
		assert!(names.iter().any(|n| n == "x-amz-date"), "must add x-amz-date");
		assert!(
	...
	fn changing_body_changes_signature() {
		...
		assert_ne!(a.signature, b.signature, "different body bytes must yield different signatures");
	}
