...
pub enum ParseError {
	...
	#[error("implausible prelude lengths: total_length={total_length}, headers_length={headers_length}")]
	ImplausiblePreludeLengths { total_length: u32, headers_length: u32 },
...
impl EventStreamParser {
	...
	pub fn next_message(&mut self) -> Result<Option<EventStreamMessage>, ParseError> {
		...
		if (total_length as usize) < PRELUDE_LEN + headers_length as usize + MESSAGE_CRC_LEN {
			return Err(ParseError::ImplausiblePreludeLengths { total_length, headers_length });
		}
		if (total_length as usize) > self.max_message_bytes {
			return Err(ParseError::MessageTooLarge { total_length, cap: self.max_message_bytes });
		}
		...
		if matches!(self.crc_validation, CrcValidation::Yes) {
			...
			if computed != prelude_crc_expected {
				return Err(ParseError::PreludeCrcMismatch { expected: prelude_crc_expected, got: computed });
			}
		}
		...
		if matches!(self.crc_validation, CrcValidation::Yes) {
			...
			if computed != message_crc_expected {
				return Err(ParseError::MessageCrcMismatch { expected: message_crc_expected, got: computed });
			}
		}
...
pub fn parse(bytes: &[u8]) -> Result<EventStreamMessage, ParseError> {
	...
	match parser.next_message()? {
		Some(msg) => Ok(msg),
		None => Err(ParseError::ImplausiblePreludeLengths { total_length: 0, headers_length: 0 }),
	}
}
...
fn parse_headers(slice: &Bytes) -> Result<HashMap<String, HeaderValue>, ParseError> {
	...
	while cursor < total {
		...
		if cursor + 1 > total {
			return Err(ParseError::TruncatedHeader { offset: cursor, needed: 1 });
		}
		let name_len = slice[cursor] as usize;
		cursor += 1;
		if cursor + name_len > total {
			return Err(ParseError::TruncatedHeader { offset: cursor, needed: name_len });
		}
		let name = std::str::from_utf8(&slice[cursor..cursor + name_len])
			.map_err(|e| ParseError::HeaderNameNotUtf8 { offset: cursor, source: e })?
			.to_ascii_lowercase();
		cursor += name_len;
		if cursor + 1 > total {
			return Err(ParseError::TruncatedHeader { offset: cursor, needed: 1 });
		...
		let value = match value_type {
			header_type::STRING => {
				if cursor + 2 > total {
					return Err(ParseError::TruncatedHeader { offset: cursor, needed: 2 });
				}
				let value_len = u16::from_be_bytes(slice[cursor..cursor + 2].try_into().expect("2 bytes")) as usize;
				cursor += 2;
				if cursor + value_len > total {
					return Err(ParseError::TruncatedHeader { offset: cursor, needed: value_len });
				}
				let s = std::str::from_utf8(&slice[cursor..cursor + value_len])
					.map_err(|e| ParseError::HeaderValueNotUtf8 { offset: cursor, source: e })?;
				cursor += value_len;
			...
			header_type::INTEGER => {
				if cursor + 4 > total {
					return Err(ParseError::TruncatedHeader { offset: cursor, needed: 4 });
				}
			...
			header_type::LONG | header_type::TIMESTAMP => {
				if cursor + 8 > total {
					return Err(ParseError::TruncatedHeader { offset: cursor, needed: 8 });
				}
			...
			header_type::UUID => {
				if cursor + 16 > total {
					return Err(ParseError::TruncatedHeader { offset: cursor, needed: 16 });
				}
			...
			other => {
				return Err(ParseError::UnsupportedHeaderType { value_type: other, offset: cursor.saturating_sub(1) });
			},
		};
...
impl MessageBuilder {
	...
	pub fn header_string(mut self, name: &str, value: &str) -> Self {
		self.headers.push((name.to_string(), HeaderValue::String(value.to_string())));
		self
	}

	pub fn header_bytes(mut self, name: &str, value: Bytes) -> Self {
		self.headers.push((name.to_string(), HeaderValue::Bytes(value)));
		self
	}
...
mod tests {
	...
	fn one_byte_at_a_time_no_loss() {
		...
		let msg = parser.next_message().unwrap().unwrap();
		assert_eq!(std::str::from_utf8(&msg.payload).unwrap(), r#"{"type":"message_stop"}"#,);
	}
	...
	fn prelude_crc_mismatch_loud() {
		...
		assert!(matches!(err, ParseError::PreludeCrcMismatch { .. }), "got {err:?}");
	}
	...
	fn message_crc_mismatch_loud() {
		...
		parser.push(&bytes);
		let err = parser.next_message().unwrap_err();
		assert!(matches!(err, ParseError::MessageCrcMismatch { .. }), "got {err:?}");
	}
	...
	fn implausible_lengths_loud() {
		...
		parser.push(&bytes);
		let err = parser.next_message().unwrap_err();
		assert!(matches!(err, ParseError::ImplausiblePreludeLengths { .. }), "got {err:?}");
	}
	...
	fn message_too_large_loud() {
		...
		parser.push(&bytes);
		let err = parser.next_message().unwrap_err();
		assert!(matches!(err, ParseError::MessageTooLarge { .. }), "got {err:?}");
	}
