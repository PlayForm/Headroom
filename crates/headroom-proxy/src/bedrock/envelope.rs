...
impl BedrockEnvelope {
	...
	pub fn parse(body: &[u8]) -> Result<Self, EnvelopeError> {
		...
		let av = obj.get(ANTHROPIC_VERSION_KEY).ok_or(EnvelopeError::MissingAnthropicVersion)?;
		let av_str = av.as_str().ok_or(EnvelopeError::AnthropicVersionNotString)?.to_string();
		Ok(Self { anthropic_version: av_str, body: value })
	}
	...
	pub fn ensure_anthropic_version_first(body: &[u8]) -> Result<Bytes, EnvelopeError> {
		...
		if map.keys().next().map(|k| k.as_str() == ANTHROPIC_VERSION_KEY).unwrap_or(false) {
		...
		// body).
		let av = map.remove(ANTHROPIC_VERSION_KEY).expect("contains_key guard above");
