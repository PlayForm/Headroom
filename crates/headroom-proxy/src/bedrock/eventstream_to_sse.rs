...
impl OutputMode {
	...
	pub fn from_accept(headers: &HeaderMap, eventstream_accept_values: &[String]) -> OutputMode {
		let accept_raw = match headers.get(http::header::ACCEPT).and_then(|v| v.to_str().ok()) {
			Some(v) => v,
...
pub fn translate_message(message: &EventStreamMessage, mode: OutputMode) -> Result<TranslateOutcome, TranslateError> {
	...
	if matches!(message.message_type(), Some("exception")) {
		let preview = String::from_utf8_lossy(&message.payload[..message.payload.len().min(160)]).into_owned();
		return Err(TranslateError::UpstreamException { payload_preview: preview });
	}

	let event_type = message.event_type().ok_or(TranslateError::MissingEventType)?;
	...
	match (mode, event_type) {
		(OutputMode::EventStream, _) => {
			...
			Ok(TranslateOutcome::Skip { event_type: event_type.to_string() })
		},
		...
		| (OutputMode::Sse, "metadata") => {
			...
			Ok(TranslateOutcome::Emit(payload_to_sse_frame(&message.payload)))
		},
		(OutputMode::Sse, other) => {
			...
			Ok(TranslateOutcome::Skip { event_type: other.to_string() })
		},
	}
}
...
mod tests {
	...
	fn accept_eventstream_selects_passthrough() {
		...
		h.insert(http::header::ACCEPT, "application/vnd.amazon.eventstream".parse().unwrap());
	...
	fn multi_accept_with_eventstream_among_them_selects_passthrough() {
		...
		h.insert(
			http::header::ACCEPT,
			"text/html, application/vnd.amazon.eventstream;q=0.9, */*".parse().unwrap(),
		);
		let mode = OutputMode::from_accept(&h, &OutputMode::default_eventstream_accept_values());
		assert_eq!(mode, OutputMode::EventStream);
	}
	...
	fn translate_converse_event_to_sse_frame() {
		let bytes = crate::bedrock::eventstream::MessageBuilder::new()
			...
			.header_string(":message-type", "event")
			.payload(Bytes::from_static(br#"{"contentBlockIndex":0,"delta":{"text":"hi"}}"#))
			.build();
		let msg = parse(&bytes).unwrap();
		let outcome = translate_message(&msg, OutputMode::Sse).unwrap();
		match outcome {
	...
	fn header_value_preview_strings() {
		assert_eq!(header_value_preview(&HeaderValue::String("hi".into())), "hi");
		assert!(header_value_preview(&HeaderValue::Bytes(Bytes::from_static(&[1, 2, 3]))).starts_with("[3 bytes"));
	}
