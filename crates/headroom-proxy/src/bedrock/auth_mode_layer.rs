...
mod tests {
	...
	async fn empty_headers_classify_as_oauth_for_bedrock() {
		...
		let app = router();
		let req = HttpRequest::builder().method("POST").uri("/probe").body(Body::empty()).unwrap();
	...
	async fn sigv4_authorization_classifies_as_oauth() {
		...
		// needed; no WARN.
		let app = router();
		let mut req = HttpRequest::builder().method("POST").uri("/probe").body(Body::empty()).unwrap();
		req.headers_mut().insert(
			"authorization",
			HeaderValue::from_static("AWS4-HMAC-SHA256 Credential=AKIA.../20260101/us-east-1/bedrock/aws4_request"),
		);
		let resp = app.oneshot(req).await.unwrap();
		let body = axum::body::to_bytes(resp.into_body(), 64).await.unwrap();
		assert_eq!(&body[..], b"oauth");
