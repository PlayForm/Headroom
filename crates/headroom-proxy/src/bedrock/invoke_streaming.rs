...
use crate::bedrock::eventstream_to_sse::{translate_message, OutputMode, TranslateError, TranslateOutcome};
use crate::bedrock::sigv4::{sign_request, SigningInputs};
use crate::compression::{compress_anthropic_request, Outcome as AnthropicOutcome, PassthroughReason};
use crate::headers::filter_response_headers;
use crate::observability::{observe_bedrock_invoke_latency, record_bedrock_eventstream_message, record_bedrock_invoke};
...
impl LatencyGuard {
	fn start(model: &str, region: &str) -> Self {
		Self { model: model.to_string(), region: region.to_string(), start: Instant::now() }
	}
}
...
pub async fn handle_invoke_streaming(
	...
	let upstream_url = match build_bedrock_streaming_upstream(&state, &model_id, &uri, action) {
		...
		Err(msg) => {
			...
			return error_response(StatusCode::INTERNAL_SERVER_ERROR, "bedrock_endpoint_invalid", &msg);
		},
	};
	...
	let extra_signed_refs: Vec<(&str, &str)> = extra_signed.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
	...
	let signed = match sign_request(&sign_inputs) {
		...
		Err(e) => {
			...
			return error_response(StatusCode::INTERNAL_SERVER_ERROR, "bedrock_sigv4_failed", &e.to_string());
		},
	};
	...
	for (name, value) in extra_signed.iter() {
		if let (Ok(n), Ok(v)) = (HeaderName::from_bytes(name.as_bytes()), http::HeaderValue::from_str(value)) {
	...
	let reqwest_method = match reqwest::Method::from_bytes(method.as_str().as_bytes()) {
		...
		Err(e) => {
			...
			return error_response(StatusCode::BAD_REQUEST, "bedrock_invalid_method", &e.to_string());
		},
	};
	...
	if !upstream_is_eventstream {
		...
		let stream = upstream_resp.bytes_stream().map(|r| r.map_err(std::io::Error::other));
	...
	let upstream_stream = upstream_resp.bytes_stream().map(|r| r.map_err(std::io::Error::other));
	...
	S: Stream<Item = Result<Bytes, std::io::Error>> + Send + 'static,
{
	...
	stream::unfold(init, |(mut parser, mut upstream, mut done, request_id, model_id, region)| {
		Box::pin(async move {
			if done {
				return None;
			}
				...
				match parser.next_message() {
					Ok(Some(msg)) => match translate_message(&msg, OutputMode::Sse) {
						Ok(TranslateOutcome::Emit(frame)) => {
							...
							return Some((Ok(frame), (parser, upstream, false, request_id, model_id, region)));
						},
						...
						Err(TranslateError::MissingEventType) => {
							...
							return Some((Ok(frame), (parser, upstream, true, request_id, model_id, region)));
						},
					},
					Ok(None) => break,
					Err(parse_err) => {
						let event_name = match &parse_err {
							ParseError::PreludeCrcMismatch { .. } | ParseError::MessageCrcMismatch { .. } => {
			...
			loop {
				match upstream.next().await {
					...
					Some(Err(e)) => {
						...
						return Some((Err(e), (parser, upstream, true, request_id, model_id, region)));
					},
					None => {
...
fn tee_to_anthropic_state<S>(upstream: S, request_id: String) -> impl Stream<Item = Result<Bytes, std::io::Error>>
where
	S: Stream<Item = Result<Bytes, std::io::Error>> + Send + 'static,
{
...
async fn run_anthropic_state_machine(mut rx: tokio::sync::mpsc::Receiver<Bytes>, request_id: String) {
...
/// single helper.
fn run_anthropic_compression(body: &Bytes, state: &AppState, _auth_mode: AuthMode, request_id: &str) -> Bytes {
...
fn build_bedrock_streaming_upstream(state: &AppState, model_id: &str, uri: &Uri, action: &str) -> Result<Url, String> {
	let base = match state.config.bedrock_endpoint.as_ref() {
		Some(u) => u.clone(),
		None => {
			let host = BEDROCK_RUNTIME_HOST_TEMPLATE.replace("{region}", &state.config.bedrock_region);
			Url::parse(&format!("https://{host}/")).map_err(|e| format!("bedrock derived base URL parse error: {e}"))?
		},
	};
	let path = format!("/model/{model_id}/{action}", model_id = model_id, action = action,);
	let mut joined = base;
...
mod tests {
	...
	fn eventstream_content_type_match() {
		...
		assert!(is_eventstream_content_type("application/vnd.amazon.eventstream; charset=utf-8"));
	...
	fn build_streaming_upstream_uses_region_default() {
		...
		let state = AppState {
			...
			vertex_token_source: std::sync::Arc::new(crate::vertex::StaticTokenSource::new("test".to_string())),
		};
		let uri: Uri = "/model/anthropic.claude-3-haiku-20240307-v1:0/invoke-with-response-stream"
			.parse()
			.unwrap();
		let url =
			build_bedrock_streaming_upstream(&state, "anthropic.claude-3-haiku-20240307-v1:0", &uri, STREAMING_ACTION)
				.unwrap();
		assert_eq!(
			url.as_str(),
	...
	fn build_streaming_upstream_supports_converse_stream_action() {
		...
		let uri: Uri = "/model/anthropic.claude-3-haiku-20240307-v1:0/converse-stream".parse().unwrap();
	...
	fn extract_streaming_action_supports_both_bedrock_paths() {
		assert_eq!(
			extract_streaming_action("/model/anthropic.claude-3-haiku-20240307-v1:0/invoke-with-response-stream"),
			Some(STREAMING_ACTION)
		);
		assert_eq!(
			extract_streaming_action("/model/anthropic.claude-3-haiku-20240307-v1:0/converse-stream"),
			Some(CONVERSE_STREAM_ACTION)
		);
		assert_eq!(
