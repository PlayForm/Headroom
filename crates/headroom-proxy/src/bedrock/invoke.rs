...
fn build_bedrock_upstream(state: &AppState, model_id: &str, uri: &Uri, action: &str) -> Result<Url, String> {
...
mod tests {
	...
	fn build_upstream_uses_region_default() {
		...
		let uri: Uri = "/model/anthropic.claude-3-haiku-20240307-v1:0/invoke".parse().unwrap();
		let url = build_bedrock_upstream(&state, "anthropic.claude-3-haiku-20240307-v1:0", &uri, "invoke").unwrap();
		assert_eq!(
			url.as_str(),
	...
	fn collect_signed_headers_strips_client_managed() {
		...
		let upstream = Url::parse("https://bedrock-runtime.us-east-1.amazonaws.com/model/x/invoke").unwrap();
		...
		let host = out.iter().find(|(k, _)| k == "host").map(|(_, v)| v.as_str()).unwrap();
