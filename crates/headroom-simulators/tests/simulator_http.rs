...
use headroom_simulators::config::{ConfiguredResponse, JsonPointerMatch, SimulatorConfig, StubRule};
...
async fn configured_stub_overrides_default_response() {
	...
	assert_eq!(response.json::<Value>().await.unwrap(), json!({"stubbed": true}));
	server.shutdown().await;
}
