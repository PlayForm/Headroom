...
fn response_from(simulated: SimulatedResponse) -> Response<Body> {
	...
	builder.body(Body::from(simulated.body)).expect("simulator response builds")
}
...
mod tests {
	...
	async fn health_route_returns_ok() {
		...
		let response = tower::ServiceExt::oneshot(app, Request::builder().uri("/healthz").body(Body::empty()).unwrap())
			.await
			.unwrap();
