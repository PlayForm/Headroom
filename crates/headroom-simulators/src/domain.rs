...
impl SimulatedResponse {
	...
	pub fn text(status: u16, content_type: &'static str, body: impl Into<Bytes>) -> Self {
		Self { status, content_type, headers: BTreeMap::new(), body: body.into() }
	}
}
...
impl RequestFacts {
	pub fn new(method: &str, path: &str, headers: &http::HeaderMap, body: Bytes) -> Self {
		...
		let wants_bedrock_eventstream =
			header_contains(headers, http::header::ACCEPT.as_str(), "application/vnd.amazon.eventstream");
		Self {
...
fn conversation_collection(method: &str) -> SimulatedResponse {
	match method {
		"POST" => SimulatedResponse::json(200, json!({"id":"conv_sim_0001","object":"conversation","metadata":{}})),
...
fn conversation_object(method: &str, path: &str) -> SimulatedResponse {
	...
	if method == "DELETE" {
		...
		SimulatedResponse::json(200, json!({"id": id, "object": "conversation", "metadata": {}}))
	}
}

fn conversation_items(method: &str, path: &str) -> SimulatedResponse {
	let id = path.trim_end_matches("/items").rsplit('/').next().unwrap_or("conv_sim_0001");
	if method == "POST" {
		SimulatedResponse::json(
			200,
...
mod tests {
	...
	fn classifies_supported_provider_paths() {
		assert_eq!(ProviderPath::classify("/v1/messages"), ProviderPath::AnthropicMessages);
		assert_eq!(
			ProviderPath::classify("/v1/chat/completions"),
			ProviderPath::OpenAiChatCompletions
		);
		assert_eq!(ProviderPath::classify("/v1/responses"), ProviderPath::OpenAiResponses);
		assert_eq!(
		...
		assert_eq!(
			ProviderPath::classify("/v1beta1/projects/p/locations/us/publishers/anthropic/models/claude:rawPredict"),
