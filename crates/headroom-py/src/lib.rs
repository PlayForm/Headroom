...
use headroom_core::transforms::smart_crusher::compaction::{ClassifyConfig, CompactConfig, DocumentCompactor};
use headroom_core::transforms::smart_crusher::{
	CrushResult as RustCrushResult, SmartCrusher as RustSmartCrusher, SmartCrusherConfig as RustSmartCrusherConfig,
};
...
use headroom_core::transforms::{
	compress_openai_responses_live_zone as rust_compress_openai_responses_live_zone, detect as rust_detect_chain,
	...
	AuthMode as RustLiveZoneAuthMode, ContentType as RustContentType, DetectionResult as RustDetectionResult,
	DiffCompressionResult, DiffCompressor, DiffCompressorConfig, DiffCompressorStats, LiveZoneOutcome,
	LogCompressionResult as RustLogResult, LogCompressor as RustLogCompressor, LogCompressorConfig as RustLogConfig,
	LogCompressorStats as RustLogStats, LogFormat as RustLogFormat, LogLevel as RustLogLevel,
...
impl PyDiffCompressionResult {
	...
	fn tokens_saved_estimate(&self) -> usize {
		let saved = self.inner.original_line_count.saturating_sub(self.inner.compressed_line_count);
		(saved * 40) / 4
	}
...
impl PyDiffCompressor {
	...
	fn new(config: Option<&PyDiffCompressorConfig>) -> Self {
		...
		Self { inner: DiffCompressor::new(cfg) }
	}
...
impl PySmartCrusher {
	...
	fn new(config: Option<&PySmartCrusherConfig>) -> Self {
		let cfg = config.map(|c| c.inner.clone()).unwrap_or_default();
		Self { inner: RustSmartCrusher::new(cfg) }
	}
	...
	fn without_compaction(config: Option<&PySmartCrusherConfig>) -> Self {
		let cfg = config.map(|c| c.inner.clone()).unwrap_or_default();
		Self { inner: RustSmartCrusher::without_compaction(cfg) }
	}
	...
	fn with_compaction_format(config: Option<&PySmartCrusherConfig>, format_name: &str) -> PyResult<Self> {
		let cfg = config.map(|c| c.inner.clone()).unwrap_or_default();
		match RustSmartCrusher::with_compaction_format(cfg, format_name) {
			...
			None => Err(pyo3::exceptions::PyValueError::new_err(format!(
				...
				headroom_core::transforms::smart_crusher::compaction::CompactionStage::SUPPORTED_FORMAT_NAMES
					.join(", ")
			))),
		}
	}
	...
	#[pyo3(signature = (content, query = "", bias = 1.0))]
	fn smart_crush_content(&self, py: Python<'_>, content: &str, query: &str, bias: f64) -> (String, bool, String) {
		let content = content.to_string();
		let query = query.to_string();
	...
	fn crush_array_json<'py>(&self, py: Python<'py>, items_json: &str, query: &str, bias: f64) -> Bound<'py, PyDict> {
		...
		let query = query.to_string();
		let (kept_json, ccr_hash, dropped_summary, strategy_info, compacted, compaction_kind) = py.detach(|| {
			let parsed: serde_json::Value =
				serde_json::from_str(&items_json).unwrap_or_else(|e| panic!("items_json must be JSON: {e}"));
			...
			let kept_json =
				serde_json::to_string(&serde_json::Value::Array(result.items)).expect("serialize kept items");
			(
				kept_json,
				result.ccr_hash,
...
fn detect_content_type(py: Python<'_>, content: &str) -> PyDetectionResult {
	...
	PyDetectionResult {
		inner: RustDetectionResult { content_type, confidence: 1.0, metadata: serde_json::Map::new() },
	}
}
...
fn score_line(line: &str, context: &str) -> Option<(Option<&'static str>, f32, f32)> {
	...
	Some((signal.category.map(category_to_str), signal.priority, signal.confidence))
}
...
impl PySearchCompressor {
	...
	fn new(config: Option<PySearchCompressorConfig>) -> Self {
		...
		Self { inner: RustSearchCompressor::new(cfg) }
	}
	...
	fn compress(&self, py: Python<'_>, content: &str, context: &str, bias: f64) -> PySearchCompressionResult {
		...
		let (result, stats) = py.detach(move || {
			...
			let (r, s) = self.inner.compress_with_store(&owned, &owned_ctx, bias, Some(&store));
			(r, s)
		});
		PySearchCompressionResult { inner: result, stats }
	}
}
...
impl PyLogCompressor {
	...
	fn new(config: Option<PyLogCompressorConfig>) -> Self {
		let cfg = config.map(|c| c.inner).unwrap_or_default();
		Self { inner: RustLogCompressor::new(cfg) }
	}
	...
	fn compress(&self, py: Python<'_>, content: &str, bias: f64) -> PyLogCompressionResult {
		...
		PyLogCompressionResult { inner: result, stats }
	}
}
...
fn protect_tags(py: Python<'_>, text: &str, compress_tagged_content: bool) -> (String, Vec<(String, String)>) {
...
fn compress_openai_responses_live_zone(
	...
	match rust_compress_openai_responses_live_zone(body, mode, model_str) {
		Ok(LiveZoneOutcome::NoChange { manifest }) => {
			...
			let transforms: Vec<String> = manifest.transforms_applied().into_iter().map(String::from).collect();
			let reason = rust_summarize_openai_responses_no_change_reason(&manifest).to_string();
			(PyBytes::new(py, body).unbind(), false, saved, transforms, Some(reason))
		},
		Ok(LiveZoneOutcome::Modified { new_body, manifest }) => {
			...
			let saved = manifest.tokens_saved() as u64;
			let transforms: Vec<String> = manifest.transforms_applied().into_iter().map(String::from).collect();
			(PyBytes::new(py, bytes).unbind(), true, saved, transforms, None)
		},
		Err(_) => {
...
impl PyTextCrusher {
	...
	fn new(config: Option<&PyTextCrusherConfig>) -> Self {
		let cfg = config.map(|c| c.inner.clone()).unwrap_or_default();
		Self { inner: RustTextCrusher::new(cfg) }
	}
	...
	fn compress(&self, py: Python<'_>, content: &str, context: &str, target_ratio: Option<f64>) -> PyTextCrusherResult {
		let content = content.to_string();
		let context = context.to_string();
