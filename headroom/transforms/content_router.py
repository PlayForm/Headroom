...
def _detect_content(content: str) -> DetectionResult:
    ...
    initialization can leave an ONNX Runtime thread alive past the Rust-side
    timeout, contending on the process-wide DLL loader lock, so the default
    backend there is the pure-Python regex detector. Set
    `HEADROOM_DETECT_BACKEND=rust` or `python` to force a backend.
...
class ContentRouter(Transform):
    ...
    def _apply_strategy_to_content(
        ...
        if compressed is not None and compressed_tokens is not None:
            ...
            if fallback_eligible_strategy and fallback_no_savings:
                ...
                # (e.g. CODE_AWARE's code-compressor-unavailable path above).
                # Prevents a duplicate strategy_chain entry and a wasted
                # second _try_ml_compressor call.
    ...
    def apply(
        ...
        **kwargs: Any,
        ...
        # so the deep edit proceeds only when the gain still beats the
        # cache-bust penalty. Block-list and non-string frozen content stay
        # frozen - the gate is wired into the string path only.
